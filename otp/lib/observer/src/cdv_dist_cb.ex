defmodule :m_cdv_dist_cb do
  ...
  def get_detail_cols(_) do
    {[{:node, 0 + 1 + 1 + 1}, {:port, 0 + 1 + 1}], true}
  end

  def get_details(id, _) do
    case :crashdump_viewer.node_info(id) do
      {:ok, info, tW} ->
        proplist =
          :crashdump_viewer.to_proplist(
            Keyword.keys(r_nod(r_nod())),
            info
          )

        title = :io_lib.format('~s (~s)', [r_nod(info, :name), id])
  ...
  def format({:creations, creations}) do
    :lists.flatten(
      :lists.join(
        ',',
        for c <- creations do
    ...
    [
      {'Overview',
       [
         ...
         {'Creation', {{:format, &format/1}, :creation}},
         {'Extra Info', :error}
       ]},
      {:scroll_boxes,
       [
