defmodule :m_cdv_proc_cb do
  ...
  def get_details(id, _) do
    case :crashdump_viewer.proc_details(id) do
      {:ok, info, tW} ->
        ...
        title = :io_lib.format('~ts (~s)', [r_proc(info, :name), id])
        {:ok, {title, proplist, tW}}

      {:error, {:other_node, nodeId}} ->
        info =
          'The process you are searching for was residing on a remote node. No process information is available. Show information about the remote node?'

        fun = fn ->
          :cdv_virtual_list_wx.start_detail_win(nodeId, :port)
        end
  ...
  def detail_pages() do
    [
      ...
      {'Messages', &init_message_page/2},
      {'Dictionary', &init_dict_page/2},
      {'Stack Dump', &init_stack_page/2},
      {'ETS tables', &init_ets_page/2},
      {'Timers', &init_timer_page/2}
    ]
  end
  ...
  defp info_fields() do
    [
      ...
      {:scroll_boxes, [{'Last Calls', 1, {:plain, :last_calls}}]},
      {:scroll_boxes,
       [
         {'Links', 1, {:click, :links}},
