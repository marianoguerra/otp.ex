defmodule :m_cdv_html_wx do
  ...
  defp init(parentWin, htmlText, tab, app) do
    ...
    {htmlWin, r_state(parent: parentWin, panel: htmlWin, expand_table: tab, app: app)}
  end
  ...
  def handle_info(
        :active,
        r_state(parent: parent, panel: htmlWin, delayed_fetch: callback) = state
      )
    ...
    :wx_misc.endBusyCursor()
    {:noreply, r_state(state, expand_table: tab, delayed_fetch: :undefined, trunc_warn: tW)}
  end
  ...
  def handle_event(
        r_wx(
          event:
            r_wxHtmlLink(
              type: :command_html_link_clicked,
              linkInfo: r_wxHtmlLinkInfo(href: target)
            )
        ),
        r_state(expand_table: tab, app: app) = state
      ) do
    newState =
      case target do
        '#Binary?' ++ binSpec ->
          [{'offset', off}, {'size', size}, {'pos', pos}] = :uri_string.dissect_query(binSpec)

          id =
            {:cdv,
             {:erlang.list_to_integer(off), :erlang.list_to_integer(size),
              :erlang.list_to_integer(pos)}}

          expand(id, :cdv_bin_cb, state)

        '#OBSBinary?' ++ binSpec ->
          [{'key1', preview}, {'key2', size}, {'key3', hash}] = :uri_string.dissect_query(binSpec)

          id =
            {:obs,
             {tab,
              {:erlang.list_to_integer(preview), :erlang.list_to_integer(size),
               :erlang.list_to_integer(hash)}}}

          expand(id, :cdv_bin_cb, state)

        '#Term?' ++ termKeys ->
          [{'key1', key1}, {'key2', key2}, {'key3', key3}] = :uri_string.dissect_query(termKeys)
  ...
  defp expand(id, callback, r_state(expand_wins: opened0, app: app) = state) do
    opened =
      case :lists.keyfind(id, 1, opened0) do
        false ->
          eW = :cdv_detail_wx.start_link(id, [], r_state(state, :panel), callback, app)
