defmodule :m_msacc do
  ...
  defp print_int(device,
            [%{"$type": :msacc_data, id: _Id} | _] = stats, options) do
  ...
  defp print_int(device,
            [%{"$type": :msacc_stats, id: _Id} | _] = stats, options) do
    :io.format(device, '~s',
                 [print_stats_header(stats, options)])
    :io.format(device, '~s',
  ...
  def stats() do
    ...
    usStats = :lists.map(fn %{counters: cnt} = m ->
                              ...
                              Map.put(%{m | counters: usCnt}, :"$type", :msacc_data)
                         end,
  ...
  defp print_stats_overview(stats, _Options) do
    ...
    realTime = div(stats(:system_realtime,
                           stats), length(stats))
    schedStats = (for (%{type: :scheduler} = s) <- stats do
                    s
                  end)
    ...
    [:io_lib.format('Average thread real-time    : ~*B us~n', [numSize, realTime]),
         :io_lib.format('Accumulated system run-time : ~*B us~n', [numSize, runTime]), :io_lib.format('Average scheduler run-time  : ~*B us~n',
                                                                 [numSize,
                                                                      avgSchedRunTime]),
                                                    :io_lib.format('~n', [])]
  end
  ...
  defp print_stats_threads(stats, options) do
    [:io_lib.format('~nStats per thread:~n', []), for thread <- stats do
                              print_thread_info(thread, options)
                            end]
  end

  defp print_stats_type(stats, options) do
    [:io_lib.format('~nStats per type:~n', []), for thread <- stats do
                              print_thread_info(thread, options)
                            end]
  end

  defp print_stats_header([%{counters: cnt} | _], %{system: printSys}) do
    [:io_lib.format('~14s', ['Thread']), map(fn counter, _ when printSys
                                                    ->
                                      :io_lib.format('~9s     ',
                                                       [:erlang.atom_to_list(counter)])
                                    counter, _ ->
                                      :io_lib.format('~9s',
                                                       [:erlang.atom_to_list(counter)])
                                 end,
  ...
  defp print_thread_info(%{"$type": :msacc_stats, counters: cnt} = thread,
            %{system: printSys}) do
    [case (:maps.find(:id, thread)) do
       ...
       {:ok, id} ->
         ...
         map(fn _Key, %{thread: threadPerc, system: systemPerc}
                    when printSys ->
  ...
  defp get_thread_perc(%{"$type": :msacc_data, counters: cnt} = thread,
    ...
    Map.merge(%{thread | "$type": :msacc_stats}, %{system:
                                             percentage(threadTime, systemTime),
                                               counters:
                                               get_thread_perc(cnt, threadTime,
                                                                 systemTime)})
  end
  ...
  defp merge_threads([%{"$type": :msacc_stats, type: type,
    ...
    case (keyfind(:type, type, acc)) do
      false ->
        merge_threads(r,
                        [:maps.remove(:id, Map.put(m0, :threads, 1)) | acc])
      %{"$type": :msacc_stats, counters: cnt0, threads: threads,
          system: system} = m ->
        newMap = %{m
                   |
  ...
  defp merge_threads([],
            [%{"$type": :msacc_stats, system: system, threads: threads,
                 counters: cnt} = m0 |
                 r]) do
