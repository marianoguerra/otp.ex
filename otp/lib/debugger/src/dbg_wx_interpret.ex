defmodule :m_dbg_wx_interpret do
  use Bitwise
  require Record
  ...
  def start(win, pos, sDir, mode) do
    title = 'Interpret Modules'

    fD =
      :dbg_wx_filedialog_win.new(win, -1, [
        {:message, title},
        {:pos, pos},
        {:defaultDir, sDir},
        {:sort, :type},
        {:filter, &filter_files/2}
      ])
    ...
    case :wxFileDialog.showModal(fD) do
      5100 ->
        ...
        send(self(), {:dbg_ui_interpret, dir})
        :ok
  ...
  defp filter_files(dir, name) do
    case :filename.extension(name) do
      '.erl' ->
        file = :filename.join(dir, name)

        case :int.interpretable(file) do
          true ->
  ...
  defp interpret_all(dir, [file0 | files], mode, window, errors) do
  ...
  defp interpret_all(_Dir, [], _Mode, _Window, []) do
    true
  end

  defp interpret_all(dir, [], _Mode, window, errors) do
    msg =
      for name <- errors do
        file = :filename.join(dir, name)
  ...
  defp format_error({:error, :no_beam}) do
    'No BEAM file'
  end

  defp format_error({:error, :no_debug_info}) do
    'No debug_info in BEAM file'
  end

  defp format_error({:error, :badarg}) do
    'File does not exist'
  end

  defp format_error({:error, {:app, app}}) do
    'Cannot interpret ' ++ :erlang.atom_to_list(app) ++ ' modules'
  end
end
