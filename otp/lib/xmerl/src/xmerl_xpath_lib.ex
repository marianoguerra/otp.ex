defmodule :m_xmerl_xpath_lib do
  use Bitwise
  require Record
  ...
  def eval(:primary_expr, primExpr, c) do
    primary_expr(primExpr, c)
  end

  def eval(:predicate, pred, c) do
    :xmerl_xpath_pred.eval(pred, c)
  end

  defp primary_expr({:number, n}, _C) do
    r_xmlObj(type: :number, value: n)
  end

  defp primary_expr({:literal, s}, _C) do
    r_xmlObj(type: :string, value: s)
  end

  defp primary_expr({:function_call, f, args}, c) do
    case :xmerl_xpath_pred.core_function(f) do
      {true, f1} ->
  ...
  defp primary_expr(primExpr, _C) do
