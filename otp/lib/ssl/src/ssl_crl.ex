defmodule :m_ssl_crl do
  ...
  def trusted_cert_and_path(cRL, {serialNumber, issuer}, {_, {db, dbRef}} = dbHandle) do
    case :ssl_pkix_db.lookup_trusted_cert(db, dbRef, serialNumber, issuer) do
      :undefined ->
  ...
  def trusted_cert_and_path(cRL, :issuer_not_found, {certPath, {db, dbRef}}) do
    case find_issuer(
           cRL,
           {:certpath,
            for der <- certPath do
  ...
  def trusted_cert_and_path(cRL, :issuer_not_found, {db, dbRef} = dbInfo) do
    case find_issuer(cRL, dbInfo) do
      {:ok, otpCert} ->
        {:ok, root, chain} = :ssl_certificate.certificate_chain(otpCert, db, dbRef)
        {:ok, root, :lists.reverse(chain)}
  ...
  defp find_issuer(cRL, {db, dbRef}) do
    issuer = :public_key.pkix_normalize_name(:public_key.pkix_crl_issuer(cRL))

    isIssuerFun = fn
      {_Key, {_Der, erlCertCandidate}}, acc ->
        verify_crl_issuer(cRL, erlCertCandidate, issuer, acc)
  ...
  defp find_issuer(isIssuerFun, :certpath, certs) do
    try do
      :lists.foldl(isIssuerFun, :issuer_not_found, certs)
    catch
      {:ok, _} = result ->
        result
    else
  ...
  defp find_issuer(isIssuerFun, :extracted, certsData) do
    certs =
  ...
  defp find_issuer(isIssuerFun, db, _) do
    try do
      :ssl_pkix_db.foldl(isIssuerFun, :issuer_not_found, db)
    catch
      {:ok, _} = result ->
        result
    else
  ...
  defp verify_crl_issuer(cRL, erlCertCandidate, issuer, notIssuer) do
    tBSCert = r_OTPCertificate(erlCertCandidate, :tbsCertificate)

    case :public_key.pkix_normalize_name(r_OTPTBSCertificate(tBSCert, :subject)) do
      ^issuer ->
        case :public_key.pkix_crl_verify(
               cRL,
               erlCertCandidate
             ) do
          true ->
