defmodule :m_ssl_pem_cache do
  ...
  def name(:dist) do
    :erlang.list_to_atom(:erlang.atom_to_list(:ssl_pem_cache) ++ '_dist')
  end

  def start_link(_) do
    cacheName = name(:normal)
    :gen_server.start_link({:local, cacheName}, :ssl_pem_cache, [cacheName], [])
  end

  def start_link_dist(_) do
    distCacheName = name(:dist)
    :gen_server.start_link({:local, distCacheName}, :ssl_pem_cache, [distCacheName], [])
  end

  def insert(file, content) do
    case bypass_cache() do
      true ->
        :ok
  ...
  def clear() do
    :erlang.put(:ssl_pem_cache, name(:normal))
    call(:unconditionally_clear_pem_cache)
  end

  defp invalidate_pem(file) do
    cast({:invalidate_pem, file})
  end

  def init([name]) do
    :erlang.put(:ssl_pem_cache, name)
  ...
  def handle_call({:unconditionally_clear_pem_cache, _}, _, r_state(pem_cache: pemCache) = state) do
    :ssl_pkix_db.clear(pemCache)
    {:reply, :ok, state}
  end

  def handle_cast(
        {:cache_pem, file, content},
        r_state(pem_cache: db) = state
      ) do
  ...
  def handle_info(
        :clear_pem_cache,
        r_state(pem_cache: pemCache, clear: interval, last_pem_check: checkPoint) = state
      ) do
    newCheckPoint = :erlang.convert_time_unit(:os.system_time(), :native, :second)
    start_pem_cache_validator(pemCache, checkPoint)
    :erlang.send_after(interval, self(), :clear_pem_cache)
    {:noreply, r_state(state, last_pem_check: newCheckPoint)}
  end
  ...
  defp call(msg) do
    :gen_server.call(:erlang.get(:ssl_pem_cache), {msg, self()}, :infinity)
  end

  defp cast(msg) do
    :gen_server.cast(:erlang.get(:ssl_pem_cache), msg)
  end

  defp start_pem_cache_validator(pemCache, checkPoint) do
    spawn_link(:ssl_pem_cache, :init_pem_cache_validator, [
      [:erlang.get(:ssl_pem_cache), pemCache, checkPoint]
    ])
  end

  def init_pem_cache_validator([cacheName, pemCache, checkPoint]) do
    :erlang.put(:ssl_pem_cache, cacheName)
    :ssl_pkix_db.foldl(&pem_cache_validate/2, checkPoint, pemCache)
  end

  defp pem_cache_validate({file, _}, checkPoint) do
    case :file.read_file_info(file, [{:time, :posix}]) do
      {:ok, r_file_info(mtime: time)} when time < checkPoint ->
        :ok
  ...
  defp bypass_cache() do
    case :application.get_env(:ssl, :bypass_pem_cache) do
