defmodule :m_tls_server_session_ticket do
  ...
  def start_link(mode, lifetime, ticketStoreSize, antiReplay) do
    :gen_server.start_link(
      :tls_server_session_ticket,
      [mode, lifetime, ticketStoreSize, antiReplay],
      []
    )
  end

  def new(pid, prf, masterSecret) do
  ...
  def handle_call(
    ...
    sessionTicket = generate_stateless_ticket(baseSessionTicket, prf, masterSecret, state)
  ...
  defp inital_state([:stateless, lifetime, _, :undefined]) do
    r_state(
      nonce: 0,
      stateless: %{
  ...
  defp inital_state([:stateless, lifetime, _, {window, k, m}]) do
    :erlang.send_after(window * 1000, self(), :rotate_bloom_filters)
  ...
  defp new_session_ticket_base(r_state(nonce: nonce, lifetime: lifetime)) do
    new_session_ticket(:undefined, nonce, lifetime)
  end

  defp new_session_ticket(id, nonce, lifetime) do
  ...
  defp stateful_ticket_store(
    ...
    case :gb_trees.size(tree0) do
      ^max ->
        {_, {r_new_session_ticket(ticket: oldRef), _, _}, tree1} = :gb_trees.take_smallest(tree0)
  ...
  defp stateless_living_ticket(obfAge, ticketAgeAdd, lifetime, timestamp, window) do
    ...
    reportedAge <= lifetime and realAge <= lifetime and
      in_window(
        realAge,
        window
      )
  ...
  defp stateless_anti_replay(index, pSK, _, state) do
    {{:ok, {index, pSK}}, state}
  end
end
