defmodule :m_tls_connection_1_3 do
  ...
  def start(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:start, :no_record, state)
  end

  def start(:internal, r_client_hello() = hello, state0, _Module) do
    case :tls_handshake_1_3.do_start(hello, state0) do
      r_alert() = alert ->
      ...
      {state, :negotiated} ->
        {:next_state, :negotiated, state,
         [{:next_event, :internal, {:start_handshake, :undefined}}]}
  ...
  def start(:internal, r_server_hello() = serverHello, state0, _Module) do
    case :tls_handshake_1_3.do_start(
           serverHello,
           state0
         ) do
      r_alert() = alert ->
  ...
  def start(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :start, state, connection)
  end

  def negotiated(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:negotiated, :no_record, state)
  end

  def negotiated(:internal, message, state0, _Module) do
    case :tls_handshake_1_3.do_negotiated(
           message,
           state0
         ) do
      r_alert() = alert ->
  ...
  def wait_cert(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:wait_cert, :no_record, state)
  end

  def wait_cert(:internal, r_certificate_1_3() = certificate, state0, _Module) do
    case :tls_handshake_1_3.do_wait_cert(
           certificate,
           state0
         ) do
      {r_alert() = alert, state} ->
  ...
  def wait_cert(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :wait_cert, state, connection)
  end

  def wait_cv(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:wait_cv, :no_record, state)
  end

  def wait_cv(:internal, r_certificate_verify_1_3() = certificateVerify, state0, _Module) do
    case :tls_handshake_1_3.do_wait_cv(
           certificateVerify,
           state0
         ) do
      {r_alert() = alert, state} ->
  ...
  def wait_cv(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :wait_cv, state, connection)
  end

  def wait_finished(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:wait_finished, :no_record, state)
  end

  def wait_finished(:internal, r_finished() = finished, state0, module) do
    case :tls_handshake_1_3.do_wait_finished(
           finished,
           state0
         ) do
      r_alert() = alert ->
      ...
      state1 ->
        ...
        :tls_connection.next_event(:connection, record, state, [{{:timeout, :handshake}, :cancel}])
    end
  end

  def wait_finished(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :wait_finished, state, connection)
  end

  def wait_sh(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:wait_sh, :no_record, state)
  end

  def wait_sh(:internal, r_server_hello() = hello, state0, _Module) do
  ...
  def wait_sh(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :wait_sh, state, connection)
  end

  def wait_ee(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:wait_ee, :no_record, state)
  end

  def wait_ee(:internal, r_encrypted_extensions() = eE, state0, _Module) do
  ...
  def wait_ee(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :wait_ee, state, connection)
  end

  def wait_cert_cr(:internal, r_change_cipher_spec(), state, _Module) do
    :tls_connection.next_event(:wait_cert_cr, :no_record, state)
  end

  def wait_cert_cr(:internal, r_certificate_1_3() = certificate, state0, _Module) do
    case :tls_handshake_1_3.do_wait_cert_cr(
           certificate,
           state0
         ) do
      {r_alert() = alert, state} ->
  ...
  def wait_cert_cr(:internal, r_certificate_request_1_3() = certificateRequest, state0, _Module) do
    case :tls_handshake_1_3.do_wait_cert_cr(
           certificateRequest,
           state0
         ) do
  ...
  def wait_cert_cr(type, msg, state, connection) do
    :ssl_connection.handle_common_event(type, msg, :wait_cert_cr, state, connection)
  end
end
