defmodule :m_ssl_config do
  ...
  defp init_private_key(_, :undefined, <<>>, _Password, _Client) do
    :undefined
  end

  defp init_private_key(dbHandle, :undefined, keyFile, password, _) do
    try do
      {:ok, list} =
  ...
  defp init_private_key(_, {asn1Type, privateKey}, _, _, _) do
    private_key(init_private_key(asn1Type, privateKey))
  end

  defp init_private_key(asn1Type, privateKey) do
  ...
  defp init_diffie_hellman(_, params, _, _) when is_binary(params) do
    :public_key.der_decode(:DHParameter, params)
  end

  defp init_diffie_hellman(_, _, _, :client) do
    :undefined
  end

  defp init_diffie_hellman(_, _, :undefined, _) do
    r_DHParameter(
  ...
  defp init_diffie_hellman(dbHandle, _, dHParamFile, :server) do
    try do
      {:ok, list} =
        :ssl_manager.cache_pem_file(
          dHParamFile,
