defmodule :m_ssl_crl_cache do
  ...
  def lookup(r_DistributionPoint(distributionPoint: {:fullName, names}), _Issuer, cRLDbInfo) do
  ...
  def select(genNames, cRLDbHandle) when is_list(genNames) do
    :lists.flatmap(
      fn
  ...
  def select(issuer, {{_Cache, mapping}, _}) do
    case :ssl_pkix_db.lookup(issuer, mapping) do
      :undefined ->
        []
  ...
  def fresh_crl(r_DistributionPoint(distributionPoint: {:fullName, names}), cRL) do
    case get_crls(names, :undefined) do
      :not_available ->
        cRL
  ...
  def insert(cRLs) do
    insert('http://dummy/no_distribution_point', cRLs)
  end

  def insert(uRI, {:file, file}) when is_list(uRI) do
    case :file.read_file(file) do
      {:ok, pemBin} ->
        pemEntries = :public_key.pem_decode(pemBin)

        cRLs =
          for {:CertificateList, cRL, :not_encrypted} <- pemEntries do
            cRL
          end
  ...
  def delete({:file, file}) do
    case :file.read_file(file) do
      {:ok, pemBin} ->
        ...
        :ssl_manager.delete_crls({'http://dummy/no_distribution_point', cRLs})
  ...
  defp get_crls(
         [
           {:uniformResourceIdentifier, 'http' ++ _ = uRL}
           | rest
         ],
         cRLDbInfo
       ) do
    case cache_lookup(uRL, cRLDbInfo) do
      [] ->
  ...
  defp get_crls([_ | rest], cRLDbInfo) do
    get_crls(rest, cRLDbInfo)
  end

  defp http_lookup(uRL, rest, cRLDbInfo, timeout) do
  ...
  defp http_get(uRL, rest, cRLDbInfo, timeout) do
    case :httpc.request(:get, {uRL, [{'connection', 'close'}]}, [{:timeout, timeout}], [
           {:body_format, :binary}
         ]) do
      {:ok, {_Status, _Headers, body}} ->
        case body do
          ...
          _ ->
            try do
              :public_key.der_decode(:CertificateList, body)
            catch
              _, _ ->
                get_crls(rest, cRLDbInfo)
            else
  ...
  defp cache_lookup(_, :undefined) do
    []
  end
  ...
  defp handle_http(uRI, rest, {_, [{:http, timeout}]} = cRLDbInfo) do
    cRLs = http_lookup(uRI, rest, cRLDbInfo, timeout)
    cRLs
  end

  defp handle_http(_, rest, cRLDbInfo) do
    get_crls(rest, cRLDbInfo)
  end
end
