defmodule :m_ssl_server_session_cache do
  use Bitwise
  ...
  Record.defrecord(:r_state, :state,
    ...
    id_generator: :undefined,
    listner: :undefined
  )

  def start_link(listner, map) do
    :gen_server.start_link(:ssl_server_session_cache, [listner, map], [])
  end

  def new_session_id(pid) do
    case call(pid, :new_session_id) do
      {:no_server, _} ->
  ...
  def reuse_session(pid, sessionId) do
    case call(pid, {:reuse_session, sessionId}) do
      {:no_server, _} ->
        :not_reusable
  ...
  def register_session(pid, session) do
  ...
  def handle_call(:new_session_id, _From, r_state(id_generator: idGen) = state) do
  ...
  def handle_call(
        ...
        _From,
        r_state(store_cb: cb, db: store0, lifetime: lifetime, session_index: index0) = state0
      ) do
    case :maps.get(sessionId, index0, :undefined) do
      :undefined ->
        {:reply, :not_reusable, state0}

      key ->
        case lookup(cb, store0, key) do
          :undefined ->
            {:reply, :not_reusable, state0}
          ...
          session ->
            case :ssl_session.valid_session(session, lifetime) do
              ...
              false ->
                ...
                {:reply, :not_reusable, r_state(state0, db: store, session_index: index)}
            end
        end
    end
  end
  ...
  def handle_cast(
        {:register_session, r_session(session_id: sessionId) = session},
        r_state(store_cb: cb, db: store0, max: max, lifetime: lifetime, session_index: index0) =
          state0
      ) do
    ...
    state =
      case size(cb, store0) do
        ^max ->
          {r_session(session_id: oldSessionId), store1} = oldest_session(cb, store0)
        ...
        size when size > 0 ->
          {r_session(session_id: oldSessionId) = oldestSession, store1} =
            oldest_session(cb, store0)

          case :ssl_session.valid_session(
                 oldestSession,
                 lifetime
               ) do
            true ->
              store = update(cb, store0, id, r_session(session, time_stamp: timeStamp))
            ...
            false ->
              store = update(cb, store1, id, r_session(session, time_stamp: timeStamp))
              index = :maps.without([oldSessionId], index0)
  ...
  def handle_info(
        {:DOWN, _, :process, listner, _},
        r_state(listner: listner) = state
      ) do
    {:stop, :normal, state}
  end
  ...
  defp session_id(key) do
    unique1 = :erlang.unique_integer()
    unique2 = :erlang.unique_integer()
    bin1 = :crypto.crypto_one_time(:aes_128_ecb, key, <<unique1::size(128)>>, true)
    bin2 = :crypto.crypto_one_time(:aes_128_ecb, key, <<unique2::size(128)>>, true)
    <<bin1::binary, bin2::binary>>
  end

  defp invalidate_session(cb, store, index, sessionId, key) do
    {delete(cb, store, key), :maps.without([sessionId], index)}
  end

  defp oldest_session(cb, store0) do
    try do
      cb.take_oldest(store0)
    catch
      _, _ ->
        {key, oldApiElement} =
          cb.foldl(
  ...
  defp init(cb, options) do
    cb.init(options)
  end

  defp lookup(cb, cache, key) do
    cb.lookup(cache, key)
  end

  defp update(:ssl_server_session_cache_db = cb, cache, key, session) do
    cb.update(cache, key, session)
  end

  defp update(cb, cache, key, session) do
    cb.update(cache, key, session)
    cache
  end

  defp delete(:ssl_server_session_cache_db = cb, cache, key) do
    cb.delete(cache, key)
  end

  defp delete(cb, cache, key) do
    cb.delete(cache, key)
    cache
  end

  defp size(cb, cache) do
    try do
      cb.size(cache)
    catch
      :error, :undef ->
