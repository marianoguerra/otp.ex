defmodule :m_tls_client_ticket_store do
  ...
  def find_ticket(pid, hashAlgos, sNI) do
    :gen_server.call(:tls_client_ticket_store, {:find_ticket, pid, hashAlgos, sNI}, :infinity)
  end
  ...
  def store_ticket(ticket, hKDF, sNI, pSK) do
    :gen_server.call(:tls_client_ticket_store, {:store_ticket, ticket, hKDF, sNI, pSK}, :infinity)
  end
  ...
  def handle_call({:find_ticket, pid, hashAlgos, sNI}, _From, state) do
  ...
  def handle_call({:store_ticket, ticket, hKDF, sNI, pSK}, _From, state0) do
  ...
  defp do_find_ticket(_, _, [], _) do
    :undefined
  end

  defp do_find_ticket(r_state(db: db, lifetime: lifetime) = state, pid, [hash | t], sNI) do
    case iterate_tickets(:gb_trees.iterator(db), pid, hash, sNI, lifetime) do
      :none ->
  ...
  defp iterate_tickets(iter0, pid, hash, sNI, lifetime) do
    case :gb_trees.next(iter0) do
      {key, r_data(hkdf: ^hash, sni: ticketSNI, timestamp: timestamp, lock: lock), iter}
  ...
  defp store_ticket(r_state(db: db0, max: max) = state, ticket, hKDF, sNI, pSK) do
    ...
    db =
      :gb_trees.insert(
        key,
        r_data(hkdf: hKDF, sni: sNI, psk: pSK, timestamp: timestamp, ticket: ticket),
        db1
      )
