defmodule :m_ssl_server_session_cache_sup do
  use Bitwise
  ...
  def start_link() do
    :supervisor.start_link({:local, tracker_name(:normal)}, :ssl_server_session_cache_sup, [])
  end

  def start_link_dist() do
    :supervisor.start_link({:local, tracker_name(:dist)}, :ssl_server_session_cache_sup, [])
  end
  ...
  defp tracker_name(:dist) do
    :erlang.list_to_atom(:erlang.atom_to_list(:ssl_server_session_cache_sup) ++ 'dist')
  end
  ...
  defp max_session_cache_size() do
    case :application.get_env(
           :ssl,
