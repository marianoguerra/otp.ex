defmodule :m_ssl_pkix_db do
  ...
  def lookup_trusted_cert(dbHandle, ref, serialNumber, issuer)
      when is_reference(ref) do
    case lookup({ref, serialNumber, issuer}, dbHandle) do
      :undefined ->
        :undefined
  ...
  def lookup_trusted_cert(_DbHandle, {:extracted, certs}, serialNumber, issuer) do
    try do
      for {:decoded, {{_Ref, certSerial, certIssuer}, cert}} <- certs,
          certSerial === serialNumber,
          certIssuer === issuer do
        throw(cert)
      end
  ...
  def add_trusted_certs(_Pid, {:extracted, _} = certs, _) do
    {:ok, certs}
  end
  ...
  def extract_trusted_certs({:der, derList}) do
    {:ok, {:extracted, certs_from_der(derList)}}
  end

  def extract_trusted_certs(file) do
    case :file.read_file(file) do
      {:ok, pemBin} ->
        content = :public_key.pem_decode(pemBin)

        derList =
          for {:Certificate, cert, :not_encrypted} <- content do
            cert
          end
  ...
  def remove_trusted_certs(ref, certsDb) do
    remove_certs(ref, certsDb)
  end

  def remove(key, db) do
    :ets.delete(db, key)
    :ok
  end

  defp remove(key, data, db) do
    :ets.delete_object(db, {key, data})
    :ok
  end

  def lookup(key, db) do
    case :ets.lookup(db, key) do
      ...
      contents ->
        ...
        for data <- contents do
          pick.(data)
        end
    end
  end

  def foldl(fun, acc0, cache) do
    :ets.foldl(fun, acc0, cache)
  end

  def select_cert_by_issuer(cache, issuer) do
    :ets.select(
      cache,
      [{{{:_, :_, issuer}, {:_, :"$1"}}, [], [:"$$"]}]
    )
  ...
  def ref_count({:extracted, _}, _Db, _N) do
    :not_cached
  end

  def ref_count(key, {db, _}, n) do
    ref_count(key, db, n)
  end

  def ref_count(key, db, n) do
  ...
  def insert(key, data, db) do
    true = :ets.insert(db, {key, data})
    :ok
  end

  defp init_ref_db(ref, file, {refDb, fileMapDb}) do
    true = :ets.insert(refDb, {ref, 1})
    true = :ets.insert(fileMapDb, {file, ref})
  end

  defp remove_certs(ref, certsDb) do
    true = :ets.match_delete(certsDb, {{ref, :_, :_}, :_})
    :ok
  end

  defp add_certs_from_der(derList, ref, certsDb) do
    add = fn cert ->
  ...
  defp certs_from_der(derList) do
    ref = make_ref()

    for cert <- derList,
        decoded <- [decode_certs(ref, cert)],
        decoded !== :undefined do
      decoded
    end
  end

  defp add_certs_from_pem(pemEntries, ref, certsDb) do
    add = fn cert ->
      add_certs(cert, ref, certsDb)
    end

    for {:Certificate, cert, :not_encrypted} <- pemEntries do
      add.(cert)
    end
  ...
  defp add_certs(cert, ref, certsDb) do
    try do
  ...
  defp decode_certs(ref, cert) do
    try do
      erlCert = :public_key.pkix_decode_cert(cert, :otp)
      tBSCertificate = r_OTPCertificate(erlCert, :tbsCertificate)
      serialNumber = r_OTPTBSCertificate(tBSCertificate, :serialNumber)
      issuer = :public_key.pkix_normalize_name(r_OTPTBSCertificate(tBSCertificate, :issuer))
      {:decoded, {{ref, serialNumber, issuer}, {cert, erlCert}}}
    catch
      :error, _ ->
        case :logger.allow(:notice, :ssl_pkix_db) do
          true ->
  ...
  defp rm_crls(cRL, mapping) do
    remove(crl_issuer(cRL), cRL, mapping)
  end

  defp crl_issuer(derCRL) do
    cRL = :public_key.der_decode(:CertificateList, derCRL)
    tBSCRL = r_CertificateList(cRL, :tbsCertList)
