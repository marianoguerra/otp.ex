defmodule :m_ssl_crl_hash_dir do
  use Bitwise
  require Record
  ...
  def lookup(r_DistributionPoint(cRLIssuer: cRLIssuer) = dP, certIssuer, cRLDbInfo) do
    case cRLIssuer do
      :asn1_NOVALUE ->
  ...
  def fresh_crl(r_DistributionPoint(), currentCRL) do
    currentCRL
  end

  def select({:rdnSequence, _} = issuer, dbHandle) do
  ...
  def select([_ | rest], cRLDbInfo) do
    select(rest, cRLDbInfo)
  end

  defp find_crls(issuer, dir) do
  ...
  defp find_crls(issuer, hash, dir, n, acc, %{:reason => reason} = report) do
    filename =
      :filename.join(
        dir,
        hash ++ '.r' ++ :erlang.integer_to_list(n)
      )
  ...
  defp maybe_parse_pem(<<"-----BEGIN", _::binary>> = pEM) do
    [{:CertificateList, dER, :not_encrypted}] = :public_key.pem_decode(pEM)
    dER
  end

  defp maybe_parse_pem(dER) when is_binary(dER) do
    dER
  end
end
