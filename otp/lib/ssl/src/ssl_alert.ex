defmodule :m_ssl_alert do
  ...
  def reason_code(r_alert(description: 0), _, _, _) do
    :closed
  end

  def reason_code(
        r_alert(description: description, role: role) = alert,
        role,
        protocolName,
        stateName
      ) do
  ...
  def reason_code(r_alert(description: description) = alert, role, protocolName, stateName) do
    {fmt, args} = alert_format(protocolName, role, stateName, alert)
    txt = :lists.flatten(:io_lib.format(fmt, args))
    {:tls_alert, {description_atom(description), txt}}
  end

  def own_alert_format(protocolName, role, stateName, alert) do
  ...
  def alert_format(protocolName, role, stateName, alert) do
    {prfixFmt, prefixArgs} = alert_prefix_format(protocolName, role, stateName)
    {fmt, args} = alert_format(alert)
    {prfixFmt ++ fmt, prefixArgs ++ args}
  end

  defp alert_prefix_format(protocolName, role, stateName) do
    {'~s ~p: In state ~p', [protocolName, role, stateName]}
  end

  defp own_alert_format(r_alert(reason: reason) = alert) do
  ...
  defp own_alert_format_depth(r_alert(reason: reason) = alert) do
    txt = own_alert_txt(alert)
  ...
  defp own_alert_txt(
    ...
    'at ' ++
      ...
      ' generated ' ++
      :string.uppercase(:erlang.atom_to_list(role)) ++
      ' ALERT: ' ++ level_txt(level) ++ description_txt(description)
  end

  defp alert_format(alert) do
    txt = alert_txt(alert)
    {' ~s\n', [txt]}
  end

  defp alert_txt(r_alert(level: level, description: description, role: role)) do
    'received ' ++
      :string.uppercase(:erlang.atom_to_list(role)) ++
      ' ALERT: ' ++ level_txt(level) ++ description_txt(description)
  end
  ...
  defp decode(
         <<level::size(8)-unsigned-big-integer, _::size(8)-unsigned-big-integer, _::binary>>,
         _,
         n
       )
       when level == 1 and n > 10 do
    r_alert(
      level: 2,
      description: 50,
  ...
  defp decode(<<_Level::size(8)-unsigned-big-integer, _::binary>>, _, _) do
    r_alert(
      level: 2,
      description: 47,
  ...
  defp description_txt(0) do
    'Close Notify'
  end

  defp description_txt(10) do
    'Unexpected Message'
  end

  defp description_txt(20) do
    'Bad Record MAC'
  end

  defp description_txt(21) do
    'Decryption Failed Reserved'
  end

  defp description_txt(22) do
    'Record Overflow'
  end

  defp description_txt(30) do
    'Decompression Failure'
  end

  defp description_txt(40) do
    'Handshake Failure'
  end

  defp description_txt(41) do
    'No Certificate Reserved'
  end

  defp description_txt(42) do
    'Bad Certificate'
  end

  defp description_txt(43) do
    'Unsupported Certificate'
  end

  defp description_txt(44) do
    'Certificate Revoked'
  end

  defp description_txt(45) do
    'Certificate Expired'
  end

  defp description_txt(46) do
    'Certificate Unknown'
  end

  defp description_txt(47) do
    'Illegal Parameter'
  end

  defp description_txt(48) do
    'Unknown CA'
  end

  defp description_txt(49) do
    'Access Denied'
  end

  defp description_txt(50) do
    'Decode Error'
  end

  defp description_txt(51) do
    'Decrypt Error'
  end

  defp description_txt(60) do
    'Export Restriction'
  end

  defp description_txt(70) do
    'Protocol Version'
  end

  defp description_txt(71) do
    'Insufficient Security'
  end

  defp description_txt(80) do
    'Internal Error'
  end

  defp description_txt(86) do
    'Inappropriate Fallback'
  end

  defp description_txt(90) do
    'User Canceled'
  end

  defp description_txt(100) do
    'No Renegotiation'
  end

  defp description_txt(109) do
    'Missing extension'
  end

  defp description_txt(110) do
    'Unsupported Extension'
  end

  defp description_txt(111) do
    'Certificate Unobtainable'
  end

  defp description_txt(112) do
    'Unrecognized Name'
  end

  defp description_txt(113) do
    'Bad Certificate Status Response'
  end

  defp description_txt(114) do
    'Bad Certificate Hash Value'
  end

  defp description_txt(115) do
    'Unknown Psk Identity'
  end

  defp description_txt(116) do
    'Certificate required'
  end

  defp description_txt(120) do
    'No application protocol'
  end

  defp description_txt(enum) do
    :lists.flatten(:io_lib.format('unsupported/unknown alert: ~p', [enum]))
  end

  defp description_atom(0) do
    :close_notify
  end

  defp description_atom(10) do
    :unexpected_message
  end

  defp description_atom(20) do
    :bad_record_mac
  end

  defp description_atom(21) do
    :decryption_failed_reserved
  end

  defp description_atom(22) do
    :record_overflow
  end

  defp description_atom(30) do
    :decompression_failure
  end

  defp description_atom(40) do
    :handshake_failure
  end

  defp description_atom(41) do
    :no_certificate_reserved
  end

  defp description_atom(42) do
    :bad_certificate
  end

  defp description_atom(43) do
    :unsupported_certificate
  end

  defp description_atom(44) do
    :certificate_revoked
  end

  defp description_atom(45) do
    :certificate_expired
  end

  defp description_atom(46) do
    :certificate_unknown
  end

  defp description_atom(47) do
    :illegal_parameter
  end

  defp description_atom(48) do
    :unknown_ca
  end

  defp description_atom(49) do
    :access_denied
  end

  defp description_atom(50) do
    :decode_error
  end

  defp description_atom(51) do
    :decrypt_error
  end

  defp description_atom(60) do
    :export_restriction
  end

  defp description_atom(70) do
    :protocol_version
  end

  defp description_atom(71) do
    :insufficient_security
  end

  defp description_atom(80) do
    :internal_error
  end

  defp description_atom(86) do
    :inappropriate_fallback
  end

  defp description_atom(90) do
    :user_canceled
  end

  defp description_atom(100) do
    :no_renegotiation
  end

  defp description_atom(109) do
    :missing_extension
  end

  defp description_atom(110) do
    :unsupported_extension
  end

  defp description_atom(111) do
    :certificate_unobtainable
  end

  defp description_atom(112) do
    :unrecognized_name
  end

  defp description_atom(113) do
    :bad_certificate_status_response
  end

  defp description_atom(114) do
    :bad_certificate_hash_value
  end

  defp description_atom(115) do
    :unknown_psk_identity
  end

  defp description_atom(116) do
    :certificate_required
  end

  defp description_atom(120) do
    :no_application_protocol
  end

  defp description_atom(_) do
