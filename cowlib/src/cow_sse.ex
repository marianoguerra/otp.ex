defmodule :cow_sse do
  use Bitwise
  require Record
  Record.defrecord(:r_state, :state, state_name: :bom,
                                 buffer: <<>>, last_event_id: <<>>,
                                 last_event_id_set: false, event_type: <<>>,
  ...
  def parse(data0,
           state = r_state(state_name: :bom, buffer: buffer)) do
    data1 = (case (buffer) do
               <<>> ->
                 data0
               _ ->
  ...
  defp process_field("id", value, state) do
    {:ok,
       r_state(state, last_event_id: value, 
  ...
  defp dispatch_event(state = r_state(last_event_id_set: false,
                      data: [])) do
    {:ok, r_state(state, event_type: <<>>)}
  end

  defp dispatch_event(state = r_state(last_event_id: lastEventID,
                      data: [])) do
  ...
  defp dispatch_event(state = r_state(last_event_id: lastEventID,
                      event_type: eventType, data: [_ | data])) do
    {:event,
  ...
  def event(event) do
    [event_comment(event), event_id(event),
  ...
  defp event_id(%{id: iD}) do
    :nomatch = :binary.match(:erlang.iolist_to_binary(iD),
                               "\n")
    ["id: ", iD, ?\n]
  end
