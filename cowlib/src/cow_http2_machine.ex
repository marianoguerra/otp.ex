defmodule :cow_http2_machine do
  ...
  Record.defrecord(:r_sendfile, :sendfile, offset: :undefined, bytes: :undefined, path: :undefined)
  ...
  Record.defrecord(:r_http2_machine, :http2_machine,
    mode: :undefined,
    opts: %{},
    state: :settings,
    preface_timer: :undefined,
    settings_timer: :undefined,
    local_settings: %{:initial_window_size => 65535},
    next_settings: :undefined,
    remote_settings: %{:initial_window_size => 65535},
    local_window: 65535,
    remote_window: 65535,
  ...
  defp start_timer(name, opts = %{:message_tag => messageTag}) do
    case :maps.get(name, opts, 5000) do
      ...
      timeout ->
        :erlang.start_timer(timeout, self(), {:cow_http2_machine, messageTag, name})
    end
  end
  ...
  defp start_timer(name, opts) do
    case :maps.get(name, opts, 5000) do
      ...
      timeout ->
        :erlang.start_timer(timeout, self(), {:cow_http2_machine, name})
    end
  end
  ...
  defp common_preface(
    ...
    case :maps.get(:initial_connection_window_size, opts, 65535) do
      65535 ->
        {:ok, :cow_http2.settings(nextSettings), state}

      size ->
        {:ok, [:cow_http2.settings(nextSettings), :cow_http2.window_update(size - 65535)],
         update_window(size - 65535, state)}
    end
  end

  defp settings_init(opts) do
    s0 = setting_from_opt(%{}, opts, :max_decode_table_size, :header_table_size, 4096)
    s1 = setting_from_opt(s0, opts, :max_concurrent_streams, :max_concurrent_streams, :infinity)
    s2 = setting_from_opt(s1, opts, :initial_stream_window_size, :initial_window_size, 65535)
    s3 = setting_from_opt(s2, opts, :max_frame_size_received, :max_frame_size, 16384)
    setting_from_opt(s3, opts, :enable_connect_protocol, :enable_connect_protocol, false)
  end

  defp setting_from_opt(settings, opts, optName, settingName, default) do
  ...
  def init_upgrade_stream(
    ...
    stream =
      r_stream(
        ...
        method: method,
        remote: :fin,
        remote_expected_size: 0,
        local_window: localWindow,
        remote_window: remoteWindow,
        te: :undefined
      )

    {:ok, 1, stream_store(stream, r_http2_machine(state, remote_streamid: 1))}
  end

  def frame(
        frame,
        state = r_http2_machine(state: :settings, preface_timer: tRef)
      ) do
    :ok =
      case tRef do
        :undefined ->
  ...
  def frame(
        frame,
        state = r_http2_machine(state: {:continuation, _, _})
      ) do
  ...
  defp data_frame(
       ...
       when (((mode === :server and rem(streamID, 2) === 0) or
                (mode === :client and rem(streamID, 2) === 1)) and streamID >= localStreamID) or
              (not ((mode === :server and rem(streamID, 2) === 0) or
                      (mode === :client and rem(streamID, 2) === 1)) and streamID > remoteStreamID) do
    {:error,
     {:connection_error, :protocol_error,
      :"DATA frame received on a stream in idle state. (RFC7540 5.1)"}, state}
  end
  ...
  defp data_frame(
         frame = {:data, streamID, _, data},
         state0 =
           r_http2_machine(
             remote_window: connWindow,
             local_lingering_streams: lingering
           )
       ) do
          ...
          false ->
            {:error,
             {:connection_error, :stream_closed,
              :"DATA frame received for a closed stream. (RFC7540 5.1)"}, state}
        end
    end
  end

  defp data_frame(
         frame = {:data, _, isFin, _},
         state0,
         stream0 =
           r_stream(id: streamID, remote_window: streamWindow, remote_read_size: streamRead),
         dataLen
       ) do
    stream =
      r_stream(stream0,
  ...
  defp is_body_size_valid(
         r_stream(remote: :nofin, remote_expected_size: expected, remote_read_size: read)
       )
  ...
  defp is_body_size_valid(
         r_stream(remote: :fin, remote_expected_size: expected, remote_read_size: expected)
       ) do
    true
  end
  ...
  Record.defrecord(:r_headers, :headers,
    id: :undefined,
    fin: :undefined,
    head: :undefined,
    data: :undefined
  ...
  defp headers_frame(
         {:headers, streamID, isFin, isHeadFin, _IsExclusive, _DepStreamID, _Weight, headerData},
         state = r_http2_machine(mode: mode)
       ) do
    headersFrame = r_headers(id: streamID, fin: isFin, head: isHeadFin, data: headerData)
  ...
  defp server_headers_frame(r_headers(id: streamID), state)
    ...
    {:error,
     {:connection_error, :protocol_error,
      :"HEADERS frame received with even-numbered streamid. (RFC7540 5.1.1)"}, state}
  end

  defp server_headers_frame(
         frame = r_headers(id: streamID, head: isHeadFin),
         state =
           r_http2_machine(
             mode: :server,
             remote_streamid: remoteStreamID
  ...
  defp server_headers_frame(
         frame = r_headers(id: streamID, fin: isFin, head: isHeadFin),
         state
       ) do
    case stream_get(streamID, state) do
      stream = r_stream(remote: :nofin) when isFin === :fin ->
        case isHeadFin do
          ...
          :head_nofin ->
            {:ok, r_http2_machine(state, state: {:continuation, :trailers, frame})}
        end
  ...
  defp client_headers_frame(
       ...
       when (rem(streamID, 2) === 1 and streamID < localStreamID) or
              (not (rem(streamID, 2) === 1) and streamID <= remoteStreamID) do
    case stream_get(streamID, state) do
      stream = r_stream(remote: :idle) ->
        case isHeadFin do
          ...
          :head_nofin ->
            {:ok, r_http2_machine(state, state: {:continuation, :response, frame})}
        end
  ...
  defp client_headers_frame(_, state) do
    {:error,
     {:connection_error, :protocol_error,
      :"HEADERS frame received on an idle stream. (RFC7540 5.1.1)"}, state}
  end

  defp headers_decode(
         frame = r_headers(head: :head_fin, data: headerData),
      ...
      _, _ ->
        {:error,
         {:connection_error, :compression_error,
          :"Error while trying to decode HPACK-encoded header block. (RFC7540 4.3)"}, state}
    else
      {headers, decodeState} when type === :request ->
        headers_enforce_concurrency_limit(
          frame,
  ...
  defp headers_enforce_concurrency_limit(
    ...
    case map_size(streams) < maxConcurrentStreams do
      ...
      false ->
        {:error,
         {:stream_error, streamID, :refused_stream,
          :"Maximum number of concurrent streams has been reached. (RFC7540 5.1.2)"}, state}
    end
  end

  defp headers_pseudo_headers(
         frame,
    ...
    try do
      ...
      intStatus ->
        response_pseudo_headers(
          tail,
          %{pseudoHeaders | :status => intStatus}
        )
    end
  end
  ...
  defp request_expected_size(
         frame = r_headers(fin: isFin),
         state,
         type,
         stream,
         pseudoHeaders,
    ...
    case (for {"content-length", cL} <- headers do
      ...
      [binLen] ->
        headers_parse_expected_size(frame, state, type, stream, pseudoHeaders, headers, binLen)
  ...
  defp response_expected_size(
         frame = r_headers(id: streamID, fin: isFin),
         state,
         type,
    ...
    case (for {"content-length", cL} <- headers do
      ...
      [_]
      when status >= 200 and status <= 299 and
             method === "CONNECT" ->
        stream_reset(
          streamID,
          state,
  ...
  defp headers_parse_expected_size(
         frame = r_headers(id: streamID),
         state,
         type,
         stream,
  ...
  defp headers_frame(
         r_headers(id: streamID, fin: isFin),
         state0 =
           r_http2_machine(
             local_settings: %{:initial_window_size => remoteWindow},
             remote_settings: %{:initial_window_size => localWindow}
    ...
    {stream, state1} =
      case type do
        :request ->
          tE =
            case :lists.keyfind("te", 1, headers) do
              {_, tE0} ->
                tE0
  ...
  defp rst_stream_frame(
    ...
    {:error,
     {:connection_error, :protocol_error,
      :"RST_STREAM frame received on a stream in idle state. (RFC7540 5.1)"}, state}
  end
  ...
  defp rst_stream_frame(
         ...
         state =
           r_http2_machine(
             streams: streams0,
             remote_lingering_streams: lingering0
           )
       ) do
  ...
  defp settings_frame(
         {:settings, settings},
         state0 = r_http2_machine(opts: opts, remote_settings: settings0)
       ) do
    state1 =
      r_http2_machine(state0,
        remote_settings:
          :maps.merge(
            settings0,
            settings
    ...
    state2 =
      :maps.fold(
        fn
          :header_table_size, newSize, state = r_http2_machine(encode_state: encodeState0) ->
            maxSize = :maps.get(:max_encode_table_size, opts, 4096)

            encodeState =
              :cow_hpack.set_max_size(
                min(
                  newSize,
                  maxSize
                ),
  ...
  defp settings_frame(_F, state = r_http2_machine(mode: :server)) do
    {:error,
     {:connection_error, :protocol_error,
      :"The preface sequence must be followed by a SETTINGS frame. (RFC7540 3.5)"}, state}
  end

  defp settings_frame(_F, state) do
    {:error,
     {:connection_error, :protocol_error,
      :"The preface must begin with a SETTINGS frame. (RFC7540 3.5)"}, state}
  end

  defp streams_update_local_window(state = r_http2_machine(streams: streams0), increment) do
    streams =
      :maps.map(
        fn _, s = r_stream(local_window: streamWindow) ->
  ...
  defp settings_ack_frame(
         state0 =
           r_http2_machine(
             settings_timer: tRef,
             local_settings: local0,
             next_settings: nextSettings
           )
       ) do
    :ok =
    ...
    {:ok,
     :maps.fold(
       fn
         :header_table_size, maxSize, state = r_http2_machine(decode_state: decodeState0) ->
           decodeState =
             :cow_hpack.set_max_size(
               maxSize,
               decodeState0
  ...
  defp streams_update_remote_window(state = r_http2_machine(streams: streams0), increment) do
    streams =
      :maps.map(
        fn _, s = r_stream(remote_window: streamWindow) ->
  ...
  Record.defrecord(:r_push_promise, :push_promise,
    id: :undefined,
    head: :undefined,
  ...
  defp push_promise_frame(_, state = r_http2_machine(mode: :server)) do
    {:error,
     {:connection_error, :protocol_error,
      :"PUSH_PROMISE frames MUST NOT be sent by the client. (RFC7540 6.6)"}, state}
  end

  defp push_promise_frame(
         _,
         state = r_http2_machine(local_settings: %{:enable_push => false})
       ) do
    {:error,
     {:connection_error, :protocol_error,
      :"PUSH_PROMISE frame received despite SETTINGS_ENABLE_PUSH set to 0. (RFC7540 6.6)"}, state}
  end

  defp push_promise_frame(
         r_push_promise(promised_id: promisedStreamID),
  ...
  defp push_promise_frame(r_push_promise(id: streamID), state)
       when not (rem(streamID, 2) === 1) do
    {:error,
     {:connection_error, :protocol_error,
      :"PUSH_PROMISE frame received on a server-initiated stream. (RFC7540 6.6)"}, state}
  end
    ...
    case stream_get(streamID, state) do
      stream = r_stream(remote: :idle) ->
        case isHeadFin do
          :head_fin ->
            headers_decode(
              r_headers(id: promisedStreamID, fin: :fin, head: isHeadFin, data: headerData),
              state,
              :push_promise,
              stream
            )
          ...
          :head_nofin ->
            {:ok, r_http2_machine(state, state: {:continuation, :push_promise, frame})}
        end
  ...
  defp window_update_frame(
         {:window_update, increment},
         state = r_http2_machine(local_window: connWindow)
       )
       when connWindow + increment > 2_147_483_647 do
    {:error,
     {:connection_error, :flow_control_error,
      :"The flow control window must not be greater than 2^31-1. (RFC7540 6.9.1)"}, state}
  end
  ...
  defp window_update_frame(
    ...
    {:error,
     {:connection_error, :protocol_error,
      :"WINDOW_UPDATE frame received on a stream in idle state. (RFC7540 5.1)"}, state}
  end

  defp window_update_frame(
         {:window_update, streamID, increment},
         state0 = r_http2_machine(remote_lingering_streams: lingering)
       ) do
    case stream_get(streamID, state0) do
      r_stream(local_window: streamWindow)
      when streamWindow + increment > 2_147_483_647 ->
        stream_reset(
          streamID,
          state0,
          :flow_control_error,
      ...
      stream0 = r_stream(local_window: streamWindow) ->
        send_data(
          r_stream(stream0, local_window: streamWindow + increment),
          state0
        )
  ...
  Record.defrecord(:r_continuation, :continuation,
    id: :undefined,
    head: :undefined,
    data: :undefined
  )
  ...
  defp continuation_frame(
         r_continuation(id: streamID, head: :head_fin, data: headerFragment1),
         state =
           r_http2_machine(
             state:
               {:continuation, type,
    ...
    headers_decode(
      ...
      r_http2_machine(state, state: :normal),
      type,
  ...
  defp continuation_frame(
         ...
         state =
           r_http2_machine(
             state:
               {:continuation, type,
                r_push_promise(id: streamID, promised_id: promisedStreamID, data: headerFragment0)}
           )
       ) do
    headerData = <<headerFragment0::binary, headerFragment1::binary>>

    headers_decode(
      r_headers(id: promisedStreamID, fin: :fin, head: :head_fin, data: headerData),
      r_http2_machine(state, state: :normal),
      type,
      :undefined
    )
  ...
  defp continuation_frame(
         r_continuation(id: streamID, data: headerFragment1),
         state = r_http2_machine(state: {:continuation, type, continuedFrame0})
       )
       when :erlang.element(2, continuedFrame0) === streamID do
    continuedFrame =
      case continuedFrame0 do
    ...
    {:ok, r_http2_machine(state, state: {:continuation, type, continuedFrame})}
  end

  defp continuation_frame(_F, state) do
    {:error,
     {:connection_error, :protocol_error,
      :"An invalid frame was received in the middle of a header block. (RFC7540 6.2)"}, state}
  end
  ...
  def timeout(:preface_timeout, tRef, state = r_http2_machine(preface_timer: tRef)) do
    {:error,
     {:connection_error, :protocol_error,
      :"The preface was not received in a reasonable amount of time."}, state}
  end

  def timeout(:settings_timeout, tRef, state = r_http2_machine(settings_timer: tRef)) do
    {:error,
     {:connection_error, :settings_timeout,
      :"The SETTINGS ack was not received within the configured time. (RFC7540 6.5.3)"}, state}
  end
  ...
  def prepare_headers(
    ...
    headers =
      merge_pseudo_headers(
        pseudoHeaders,
        remove_http11_headers(headers0)
      )

    {headerBlock, encodeState} =
      :cow_hpack.encode(
        headers,
        encodeState0
      )
    ...
    {:ok, isFin, headerBlock,
     stream_store(
       r_stream(stream, local: isFin0),
       r_http2_machine(state, encode_state: encodeState)
     )}
  end

  def prepare_push_promise(_, r_http2_machine(remote_settings: %{:enable_push => false}), _, _) do
    {:error, :no_push}
  end

  def prepare_push_promise(
        streamID,
        state =
          r_http2_machine(
            encode_state: encodeState0,
            local_settings: %{:initial_window_size => remoteWindow},
            remote_settings: %{:initial_window_size => localWindow},
            local_streamid: localStreamID
          ),
    ...
    tE =
      case :lists.keyfind("te", 1, headers0) do
        {_, tE0} ->
          tE0
  ...
  defp remove_http11_headers(headers) do
    ...
    removeHeaders =
      case :lists.keyfind("connection", 1, headers) do
        false ->
          removeHeaders0
  ...
  defp merge_pseudo_headers(pseudoHeaders, headers0) do
    :lists.foldl(
      ...
      headers0,
      :maps.to_list(pseudoHeaders)
    )
  end

  def prepare_trailers(streamID, state = r_http2_machine(encode_state: encodeState0), trailers) do
    stream =
      r_stream(local: :nofin) =
      stream_get(
        streamID,
        state
      )
    ...
    {:ok, headerBlock,
     stream_store(
       r_stream(stream, local: :fin),
       r_http2_machine(state, encode_state: encodeState)
     )}
  end

  def send_or_queue_data(
        streamID,
    ...
    stream0 =
      r_stream(local: :nofin, local_window: streamWindow, local_buffer_size: bufferSize, te: tE0) =
      stream_get(streamID, state0)

    dataOrFileOrTrailers =
      case dataOrFileOrTrailers0 do
        {:trailers, _} ->
          tE =
            try do
    ...
    cond do
      streamWindow < minSendSize and (streamWindow < sendSize or connWindow < sendSize) ->
        {:ok,
         stream_store(
           queue_data(stream0, isFin0, dataOrFileOrTrailers, :in),
           state0
         )}

      true ->
        case send_or_queue_data(stream0, state0, [], isFin0, dataOrFileOrTrailers, :in) do
          {:ok, stream, state, []} ->
  ...
  defp send_data(state0 = r_http2_machine(streams: streams0)) do
    ...
    case send_data_for_all_streams(:maps.next(iterator), streams0, state0, []) do
      {:ok, streams, state, []} ->
  ...
  defp send_data_for_all_streams(
         _,
         streams,
         state = r_http2_machine(local_window: connWindow),
         send
  ...
  defp send_data_for_all_streams({streamID, stream0, iterator}, streams, state0, send) do
    case send_data_for_one_stream(stream0, state0, []) do
      {:ok, stream, state, []} ->
        send_data_for_all_streams(
          :maps.next(iterator),
      ...
      {:ok, r_stream(local: :fin, remote: :fin), state, sendData} ->
        send_data_for_all_streams(:maps.next(iterator), :maps.remove(streamID, streams), state, [
          {streamID, :fin, sendData} | send
        ])

      {:ok, stream = r_stream(local: isFin), state, sendData} ->
        send_data_for_all_streams(:maps.next(iterator), %{streams | streamID => stream}, state, [
  ...
  defp send_data(stream0, state0) do
    case send_data_for_one_stream(stream0, state0, []) do
      ...
      {:ok, stream = r_stream(id: streamID, local: isFin), state, sendData} ->
        {:send, [{streamID, isFin, sendData}], stream_store(stream, state)}
    end
  end

  defp send_data_for_one_stream(
         stream = r_stream(local: :nofin, local_buffer_size: 0, local_trailers: trailers),
         state,
         sendAcc
       )
  ...
  defp send_data_for_one_stream(
         stream = r_stream(local: :nofin, local_buffer: q0, local_buffer_size: 0),
         state,
         sendAcc
       ) do
    case :queue.len(q0) do
      ...
      1 ->
        ...
        {:ok, r_stream(stream, local: :fin, local_buffer: q), state, :lists.reverse(sendAcc)}
    end
  end
  ...
  defp send_data_for_one_stream(
         stream0 =
           r_stream(local_window: streamWindow, local_buffer: q0, local_buffer_size: bufferSize),
         state0 = r_http2_machine(opts: opts, local_window: connWindow),
         sendAcc0
       ) do
    minSendSize = :maps.get(:stream_window_data_threshold, opts, 16384)
    ...
    cond do
      sendAcc0 === [] and streamWindow < minSendSize and
          (streamWindow < bufferSize or connWindow < bufferSize) ->
        {:ok, stream0, state0, []}
  ...
  defp send_or_queue_data(
         stream = r_stream(local_buffer_size: 0),
         state,
         sendAcc,
         :fin,
         {:trailers, trailers},
  ...
  defp send_or_queue_data(stream, state, sendAcc, :fin, {:trailers, trailers}, _) do
    {:ok, r_stream(stream, local_trailers: trailers), state, sendAcc}
  end

  defp send_or_queue_data(
         stream = r_stream(local_window: streamWindow),
         state = r_http2_machine(local_window: connWindow),
         sendAcc,
         isFin,
         data,
    ...
    {:ok, queue_data(stream, isFin, data, in__), state, sendAcc}
  end

  defp send_or_queue_data(
         stream = r_stream(local_window: streamWindow),
         state =
           r_http2_machine(opts: opts, remote_settings: remoteSettings, local_window: connWindow),
         sendAcc,
         isFin,
         data,
         in__
    ...
    case data do
      file = r_sendfile(bytes: bytes) when bytes <= maxSendSize ->
        {:ok,
         ...
         ), r_http2_machine(state, local_window: connWindow - bytes), [file | sendAcc]}

      file = r_sendfile(offset: offset, bytes: bytes) ->
        send_or_queue_data(
          r_stream(stream, local_window: streamWindow - maxSendSize),
          r_http2_machine(state, local_window: connWindow - maxSendSize),
          [r_sendfile(file, bytes: maxSendSize) | sendAcc],
          isFin,
          r_sendfile(file,
            offset: offset + maxSendSize,
  ...
  def ensure_window(
        size,
        state = r_http2_machine(opts: opts, remote_window: remoteWindow)
      ) do
    case ensure_window(size, remoteWindow, :connection, opts) do
      :ok ->
        :ok

      {:ok, increment} ->
        {:ok, increment, r_http2_machine(state, remote_window: remoteWindow + increment)}
    end
  end
  ...
  def ensure_window(streamID, size, state = r_http2_machine(opts: opts)) do
    case stream_get(streamID, state) do
      ...
      stream = r_stream(remote_window: remoteWindow) ->
        case ensure_window(size, remoteWindow, :stream, opts) do
          :ok ->
            :ok
          ...
          {:ok, increment} ->
            {:ok, increment,
             stream_store(
               r_stream(stream, remote_window: remoteWindow + increment),
               state
             )}
        end
    end
  ...
  defp ensure_window_max(:connection, opts) do
    :maps.get(:max_connection_window_size, opts, 2_147_483_647)
  end

  defp ensure_window_max(:stream, opts) do
    :maps.get(:max_stream_window_size, opts, 2_147_483_647)
  end

  defp ensure_window_threshold(:connection, opts) do
    :maps.get(:connection_window_update_threshold, opts, 163_840)
  end

  defp ensure_window_threshold(:stream, opts) do
    :maps.get(:stream_window_update_threshold, opts, 163_840)
  end
  ...
  def update_window(streamID, size, state) when size > 0 do
    stream =
      r_stream(remote_window: remoteWindow) =
      stream_get(
        streamID,
        state
      )
    ...
    stream_store(
      r_stream(stream, remote_window: remoteWindow + size),
      state
    )
  end
  ...
  def reset_stream(streamID, state = r_http2_machine(streams: streams0)) do
    case :maps.take(streamID, streams0) do
      {_, streams} ->
        {:ok, stream_linger(streamID, r_http2_machine(state, streams: streams))}
  ...
  def get_connection_local_buffer_size(r_http2_machine(streams: streams)) do
    :maps.fold(
      fn _, r_stream(local_buffer_size: size), acc ->
        acc + size
      end,
      0,
      streams
    ...
    defaults =
      case mode do
        :server ->
          %{
            defaults0
            | :enable_connect_protocol => default_setting_value(:enable_connect_protocol)
          }
  ...
  def get_stream_local_buffer_size(
        streamID,
        state =
          r_http2_machine(
            mode: mode,
    ...
    case stream_get(streamID, state) do
      ...
      when (((mode === :server and rem(streamID, 2) === 0) or
               (mode === :client and rem(streamID, 2) === 1)) and streamID < localStreamID) or
             (not ((mode === :server and rem(streamID, 2) === 0) or
                       (mode === :client and rem(streamID, 2) === 1)) and
  ...
  def get_stream_local_state(
    ...
    case stream_get(streamID, state) do
      r_stream(local: isFin, local_buffer: q, local_trailers: :undefined) ->
        isQueueFin =
  ...
  def is_lingering_stream(
        streamID,
        r_http2_machine(
          local_lingering_streams: local,
          remote_lingering_streams: remote
        )
      ) do
  ...
  defp stream_store(
         r_stream(id: streamID, local: :fin, remote: :fin),
  ...
  defp stream_store(
         stream = r_stream(id: streamID),
         state = r_http2_machine(streams: streams)
       ) do
    r_http2_machine(state, streams: %{streams | streamID => stream})
  end

  defp stream_reset(streamID, state, reason, humanReadable) do
    {:error, {:stream_error, streamID, reason, humanReadable}, stream_linger(streamID, state)}
  end

  defp stream_linger(
         streamID,
         state = r_http2_machine(local_lingering_streams: lingering0)
       ) do
    lingering = [
      streamID
      | :lists.sublist(
