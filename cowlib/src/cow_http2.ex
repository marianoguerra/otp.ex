defmodule :cow_http2 do
  ...
  def parse(<<_ :: size(24), 0 :: size(8), _ :: size(9),
             0 :: size(31), _ :: bits>>) do
    {:connection_error, :protocol_error, :"DATA frames MUST be associated with a stream. (RFC7540 6.1)"}
  end

  def parse(<<0 :: size(24), 0 :: size(8), _ :: size(4),
             1 :: size(1), _ :: size(35), _ :: bits>>) do
    {:connection_error, :frame_size_error, :"DATA frames with padding flag MUST have a length > 0. (RFC7540 6.1)"}
  end

  def parse(<<len0 :: size(24), 0 :: size(8), _ :: size(4),
             1 :: size(1), _ :: size(35), padLen :: size(8),
             _ :: bits>>)
      when padLen >= len0 do
    {:connection_error, :protocol_error, :"Length of padding MUST be less than length of payload. (RFC7540 6.1)"}
  end

  def parse(<<len :: size(24), 0 :: size(8), _ :: size(4),
             0 :: size(1), _ :: size(2), flagEndStream :: size(1),
             _ :: size(1), streamID :: size(31),
             data :: size(len) - binary, rest :: bits>>) do
    {:ok, {:data, streamID, parse_fin(flagEndStream), data},
       rest}
  end

  def parse(<<len0 :: size(24), 0 :: size(8), _ :: size(4),
             1 :: size(1), _ :: size(2), flagEndStream :: size(1),
             _ :: size(1), streamID :: size(31), padLen :: size(8),
             rest0 :: bits>>)
      when byte_size(rest0) >= len0 - 1 do
    len = len0 - padLen - 1
    case (rest0) do
      <<data :: size(len) - binary,
          0 :: size(padLen) - unit(8), rest :: bits>> ->
        {:ok, {:data, streamID, parse_fin(flagEndStream), data},
           rest}
      _ ->
  ...
  def parse(<<len0 :: size(24), 1 :: size(8), _ :: size(2),
             1 :: size(1), _ :: size(1), 1 :: size(1), _ :: size(35),
  ...
  def parse(<<len :: size(24), 1 :: size(8), _ :: size(2),
             0 :: size(1), _ :: size(1), 0 :: size(1),
             flagEndHeaders :: size(1), _ :: size(1),
             flagEndStream :: size(1), _ :: size(1),
             streamID :: size(31),
             headerBlockFragment :: size(len) - binary,
             rest :: bits>>) do
    {:ok,
       {:headers, streamID, parse_fin(flagEndStream),
          parse_head_fin(flagEndHeaders), headerBlockFragment},
       rest}
  end

  def parse(<<len0 :: size(24), 1 :: size(8), _ :: size(2),
             0 :: size(1), _ :: size(1), 1 :: size(1),
             flagEndHeaders :: size(1), _ :: size(1),
             flagEndStream :: size(1), _ :: size(1),
             streamID :: size(31), padLen :: size(8),
             rest0 :: bits>>)
      when byte_size(rest0) >= len0 - 1 do
    len = len0 - padLen - 1
    case (rest0) do
  ...
  def parse(<<_ :: size(24), 1 :: size(8), _ :: size(2),
             1 :: size(1), _ :: size(1), 0 :: size(1), _ :: size(4),
             streamID :: size(31), _ :: size(1),
             streamID :: size(31), _ :: bits>>) do
    {:connection_error, :protocol_error, :"HEADERS frames cannot define a stream that depends on itself. (RFC7540 5.3.1)"}
  end

  def parse(<<len0 :: size(24), 1 :: size(8), _ :: size(2),
             1 :: size(1), _ :: size(1), 0 :: size(1),
             flagEndHeaders :: size(1), _ :: size(1),
             flagEndStream :: size(1), _ :: size(1),
             streamID :: size(31), e :: size(1),
    ...
    {:ok,
       {:headers, streamID, parse_fin(flagEndStream),
          parse_head_fin(flagEndHeaders), parse_exclusive(e),
          depStreamID, weight + 1, headerBlockFragment},
       rest}
  end

  def parse(<<_ :: size(24), 1 :: size(8), _ :: size(2),
             1 :: size(1), _ :: size(1), 1 :: size(1), _ :: size(4),
             streamID :: size(31), _ :: size(9),
             streamID :: size(31), _ :: bits>>) do
    {:connection_error, :protocol_error, :"HEADERS frames cannot define a stream that depends on itself. (RFC7540 5.3.1)"}
  end

  def parse(<<len0 :: size(24), 1 :: size(8), _ :: size(2),
             1 :: size(1), _ :: size(1), 1 :: size(1),
             flagEndHeaders :: size(1), _ :: size(1),
             flagEndStream :: size(1), _ :: size(1),
             streamID :: size(31), padLen :: size(8), e :: size(1),
             depStreamID :: size(31), weight :: size(8),
             rest0 :: bits>>)
  ...
  def parse(<<5 :: size(24), 2 :: size(8), _ :: size(9),
             0 :: size(31), _ :: bits>>) do
  ...
  def parse(<<5 :: size(24), 2 :: size(8), _ :: size(9),
             streamID :: size(31), _ :: size(1),
             streamID :: size(31), _ :: size(8), rest :: bits>>) do
    {:stream_error, streamID, :protocol_error, :"PRIORITY frames cannot make a stream depend on itself. (RFC7540 5.3.1)", rest}
  end
  ...
  def parse(<<len0 :: size(24), 5 :: size(8), _ :: size(4),
             0 :: size(1), flagEndHeaders :: size(1), _ :: size(3),
             streamID :: size(31), _ :: size(1),
             promisedStreamID :: size(31), rest0 :: bits>>)
    ...
    {:ok,
       {:push_promise, streamID,
          parse_head_fin(flagEndHeaders), promisedStreamID,
          headerBlockFragment},
       rest}
  end

  def parse(<<len0 :: size(24), 5 :: size(8), _ :: size(4),
             1 :: size(1), flagEndHeaders :: size(1), _ :: size(2),
             streamID :: size(31), padLen :: size(8), _ :: size(1),
             promisedStreamID :: size(31), rest0 :: bits>>)
      when byte_size(rest0) >= len0 - 5 do
    len = len0 - 5
    case (rest0) do
  ...
  def parse(<<8 :: size(24), 6 :: size(8), _ :: size(7),
             1 :: size(1), _ :: size(1), 0 :: size(31),
             opaque :: size(64), rest :: bits>>) do
  ...
  def parse(<<8 :: size(24), 6 :: size(8), _ :: size(104),
             _ :: bits>>) do
  ...
  def parse(<<4 :: size(24), 8 :: size(8), _ :: size(9),
             streamID :: size(31), _ :: size(1),
             increment :: size(31), rest :: bits>>) do
  ...
  def parse(<<_ :: size(24), 9 :: size(8), _ :: size(9),
             0 :: size(31), _ :: bits>>) do
    {:connection_error, :protocol_error, :"CONTINUATION frames MUST be associated with a stream. (RFC7540 6.10)"}
  end

  def parse(<<len :: size(24), 9 :: size(8), _ :: size(5),
             flagEndHeaders :: size(1), _ :: size(3),
             streamID :: size(31),
             headerBlockFragment :: size(len) - binary,
             rest :: bits>>) do
    {:ok,
  ...
  def parse(<<len :: size(24), type :: size(8),
             _ :: size(40), _ :: size(len) - binary, rest :: bits>>)
      when type > 9 do
    {:ignore, rest}
  end
  ...
  defp parse_settings_payload(<<1 :: size(16), value :: size(32),
              rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :header_table_size, value))
  end

  defp parse_settings_payload(<<2 :: size(16), 0 :: size(32), rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :enable_push, false))
  end

  defp parse_settings_payload(<<2 :: size(16), 1 :: size(32), rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :enable_push, true))
  end

  defp parse_settings_payload(<<2 :: size(16), _ :: size(32), _ :: bits>>, _,
            _) do
    {:connection_error, :protocol_error, :"The SETTINGS_ENABLE_PUSH value MUST be 0 or 1. (RFC7540 6.5.2)"}
  end

  defp parse_settings_payload(<<3 :: size(16), value :: size(32),
              rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :max_concurrent_streams, value))
  end

  defp parse_settings_payload(<<4 :: size(16), value :: size(32), _ :: bits>>,
            _, _)
      when value > 2147483647 do
    {:connection_error, :flow_control_error, :"The maximum SETTINGS_INITIAL_WINDOW_SIZE value is 0x7fffffff. (RFC7540 6.5.2)"}
  end

  defp parse_settings_payload(<<4 :: size(16), value :: size(32),
              rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :initial_window_size, value))
  end

  defp parse_settings_payload(<<5 :: size(16), value :: size(32), _ :: bits>>,
            _, _)
      when value <= 16383 do
    {:connection_error, :protocol_error, :"The SETTINGS_MAX_FRAME_SIZE value must be > 0x3fff. (RFC7540 6.5.2)"}
  end

  defp parse_settings_payload(<<5 :: size(16), value :: size(32),
              rest :: bits>>,
            len, settings)
      when value <= 16777215 do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :max_frame_size, value))
  end

  defp parse_settings_payload(<<5 :: size(16), _ :: size(32), _ :: bits>>, _,
            _) do
    {:connection_error, :protocol_error, :"The SETTINGS_MAX_FRAME_SIZE value must be =< 0xffffff. (RFC7540 6.5.2)"}
  end

  defp parse_settings_payload(<<6 :: size(16), value :: size(32),
              rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :max_header_list_size, value))
  end

  defp parse_settings_payload(<<8 :: size(16), 0 :: size(32), rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :enable_connect_protocol, false))
  end

  defp parse_settings_payload(<<8 :: size(16), 1 :: size(32), rest :: bits>>,
            len, settings) do
    parse_settings_payload(rest, len - 6,
                             Map.put(settings, :enable_connect_protocol, true))
  end

  defp parse_settings_payload(<<8 :: size(16), _ :: size(32), _ :: bits>>, _,
            _) do
    {:connection_error, :protocol_error, :"The SETTINGS_ENABLE_CONNECT_PROTOCOL value MUST be 0 or 1. (draft-h2-websockets-01 3)"}
  end

  defp parse_settings_payload(<<_ :: size(48), rest :: bits>>, len,
            settings) do
    parse_settings_payload(rest, len - 6, settings)
  end

  def data(streamID, isFin, data) do
    [data_header(streamID, isFin,
  ...
  def data_header(streamID, isFin, len) do
    ...
    <<len :: size(24), 0 :: size(15),
        flagEndStream :: size(1), 0 :: size(1),
        streamID :: size(31)>>
  end
  ...
  def headers(streamID, isFin, headerBlock) do
    ...
    flagEndStream = flag_fin(isFin)
    flagEndHeaders = 1
    [<<len :: size(24), 1 :: size(8), 0 :: size(5),
         flagEndHeaders :: size(1), 0 :: size(1),
         flagEndStream :: size(1), 0 :: size(1),
         streamID :: size(31)>>,
         headerBlock]
  end
  ...
  def priority(streamID, e, depStreamID, weight) do
    ...
    <<5 :: size(24), 2 :: size(8), 0 :: size(9),
        streamID :: size(31), flagExclusive :: size(1),
        depStreamID :: size(31), weight :: size(8)>>
  end

  def rst_stream(streamID, reason) do
    errorCode = error_code(reason)
    <<4 :: size(24), 3 :: size(8), 0 :: size(9),
        streamID :: size(31), errorCode :: size(32)>>
  end
  ...
  def settings(settings) do
    ...
    [<<len :: size(24), 4 :: size(8), 0 :: size(40)>>,
         payload]
  end
  ...
  def push_promise(streamID, promisedStreamID, headerBlock) do
    ...
    flagEndHeaders = 1
    [<<len :: size(24), 5 :: size(8), 0 :: size(5),
         flagEndHeaders :: size(1), 0 :: size(3),
         streamID :: size(31), 0 :: size(1),
         promisedStreamID :: size(31)>>,
         headerBlock]
  end

  def ping(opaque) do
    <<8 :: size(24), 6 :: size(8), 0 :: size(40),
        opaque :: size(64)>>
  end

  def ping_ack(opaque) do
    <<8 :: size(24), 6 :: size(8), 0 :: size(7),
        1 :: size(1), 0 :: size(32), opaque :: size(64)>>
  end

  def goaway(lastStreamID, reason, debugData) do
    errorCode = error_code(reason)
    len = :erlang.iolist_size(debugData) + 8
    [<<len :: size(24), 7 :: size(8), 0 :: size(41),
         lastStreamID :: size(31), errorCode :: size(32)>>,
         debugData]
  end
  ...
  def window_update(streamID, increment)
      when increment <= 2147483647 do
    <<4 :: size(24), 8 :: size(8), 0 :: size(8),
        streamID :: size(32), 0 :: size(1),
        increment :: size(31)>>
  end
