defmodule :cow_http_struct_hd do
  ...
  def parse_dictionary(<<c, r::bits>>)
      when c === ?a or c === ?b or c === ?c or c === ?d or c === ?e or c === ?f or c === ?g or
             ...
             c === ?v or c === ?w or c === ?x or c === ?y or c === ?z do
  ...
  defp parse_dict_key(<<c, r::bits>>, acc, order, k)
       when c === ?a or c === ?b or c === ?c or c === ?d or c === ?e or c === ?f or c === ?g or
              c === ?h or c === ?i or c === ?j or c === ?k or c === ?l or c === ?m or c === ?n or
              c === ?o or c === ?p or c === ?q or c === ?r or c === ?s or c === ?t or c === ?u or
              c === ?v or c === ?w or c === ?x or c === ?y or c === ?z or c === ?0 or c === ?1 or
              c === ?2 or c === ?3 or c === ?4 or c === ?5 or c === ?6 or c === ?7 or c === ?8 or
              c === ?9 or c === ?_ or c === ?- or c === ?* do
  ...
  defp parse_float(r, l1, l2, acc)
       when (l1 <= 9 and l2 <= 6) or
              (l1 <= 10 and l2 <= 5) or
              (l1 <= 11 and l2 <= 4) or
              (l1 <= 12 and l2 <= 3) or
              (l1 <= 13 and l2 <= 2) or
  ...
  defp parse_string(<<c, r::bits>>, acc)
       when (c >= 32 and
               c <= 33) or
              (c >= 35 and c <= 91) or
              (c >= 93 and c <= 126) do
  ...
  defp bare_item(float) when is_float(float) do
    decimals =
      case trunc(float) do
        i when i >= 10_000_000_000_000 ->
          1

        i when i >= 1_000_000_000_000 ->
          2

        i when i >= 100_000_000_000 ->
          3

        i when i >= 10_000_000_000 ->
          4

        i when i >= 1_000_000_000 ->
          5
