defmodule :cow_ws do
  ...
  defp negotiate_permessage_deflate1(params, extensions, opts) do
    ...
    case negotiate_params(params, negotiated0, respParams1) do
      :ignore ->
        :ignore
  ...
  defp negotiate_params(["client_max_window_bits" | tail], negotiated, respParams) do
    ...
    negotiate_params(
      tail,
      %{negotiated | :client_max_window_bits_set => true},
  ...
  defp negotiate_params([{"server_max_window_bits", max} | tail], negotiated, respParams) do
    ...
    case parse_max_window_bits(max) do
      ...
      sB when sB <= sB0 ->
        negotiate_params(
          tail,
          %{negotiated | :server_max_window_bits => sB, :server_max_window_bits_set => true},
  ...
  defp negotiate_params(["client_no_context_takeover" | tail], negotiated, respParams) do
    case :maps.get(
      ...
      :takeover ->
        negotiate_params(
          tail,
          %{negotiated | :client_context_takeover => :no_takeover},
  ...
  defp negotiate_params(["server_no_context_takeover" | tail], negotiated, respParams) do
    case :maps.get(
      ...
      :takeover ->
        negotiate_params(
          tail,
          %{negotiated | :server_context_takeover => :no_takeover},
  ...
  def validate_permessage_deflate(params, extensions, opts) do
    case :lists.usort(params) do
      ...
      params2 ->
        case parse_response_permessage_deflate_params(params2, 15, :takeover, 15, :takeover) do
          :error ->
            :error
  ...
  defp parse_response_permessage_deflate_params(
         [{"client_max_window_bits", max} | tail],
         _,
         cTO,
         sB,
         sTO
    ...
    case parse_max_window_bits(max) do
      ...
      cB ->
        parse_response_permessage_deflate_params(tail, cB, cTO, sB, sTO)
    end
  end

  defp parse_response_permessage_deflate_params(
         ["client_no_context_takeover" | tail],
         cB,
         _,
         sB,
         sTO
  ...
  defp parse_response_permessage_deflate_params(
         [{"server_max_window_bits", max} | tail],
         cB,
         cTO,
         _,
         sTO
  ...
  defp parse_response_permessage_deflate_params(
         ["server_no_context_takeover" | tail],
         cB,
         cTO,
         sB,
         _
  ...
  def parse_header(<<_::size(1), rsv::size(3), _::bits>>, extensions, _)
      when extensions === %{} and rsv !== 0 do
    :error
  end

  def parse_header(<<_::size(2), 1::size(1), _::bits>>, %{:deflate => _}, _) do
    :error
  end

  def parse_header(<<_::size(3), 1::size(1), _::bits>>, %{:deflate => _}, _) do
    :error
  end

  def parse_header(<<_::size(4), 3::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 4::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 5::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 6::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 7::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 11::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 12::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 13::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 14::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(4), 15::size(4), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<0::size(1), _::size(3), opcode::size(4), _::bits>>, _, _)
      when opcode >= 8 do
    :error
  end

  def parse_header(<<_::size(4), 0::size(4), _::bits>>, _, :undefined) do
    :error
  end

  def parse_header(<<_::size(4), 1::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 2::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 3::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 4::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 5::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 6::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 7::size(4), _::bits>>, _, {_, _, _}) do
    :error
  end

  def parse_header(<<_::size(4), 8::size(4), _::size(1), 1::size(7), _::bits>>, _, _) do
    :error
  end

  def parse_header(data = <<_::size(4), 8::size(4), 0::size(1), len::size(7), _::bits>>, _, _)
      when len > 1 and byte_size(data) < 4 do
    :more
  end

  def parse_header(data = <<_::size(4), 8::size(4), 1::size(1), len::size(7), _::bits>>, _, _)
      when len > 1 and byte_size(data) < 8 do
    :more
  end

  def parse_header(
        <<fin::size(1), rsv::size(3)-bits, opcode::size(4), 0::size(1), len::size(7),
          rest::bits>>,
        _,
        fragState
      )
  ...
  def parse_header(
        <<fin::size(1), rsv::size(3)-bits, opcode::size(4), 1::size(1), len::size(7),
  ...
  def parse_header(
        <<fin::size(1), rsv::size(3)-bits, opcode::size(4), 0::size(1), 126::size(7),
  ...
  def parse_header(
        <<fin::size(1), rsv::size(3)-bits, opcode::size(4), 1::size(1), 126::size(7),
  ...
  def parse_header(
        <<fin::size(1), rsv::size(3)-bits, opcode::size(4), 0::size(1), 127::size(7), 0::size(1),
          len::size(63), rest::bits>>,
        _,
        fragState
      )
  ...
  def parse_header(
        <<fin::size(1), rsv::size(3)-bits, opcode::size(4), 1::size(1), 127::size(7), 0::size(1),
          len::size(63), maskKey::size(32), rest::bits>>,
        _,
        fragState
      )
      when len > 65535 and opcode < 8 do
  ...
  def parse_header(<<_::size(9), 127::size(7), 1::size(1), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(8), 0::size(1), 126::size(7), _::size(16), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(8), 1::size(1), 126::size(7), _::size(48), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(8), 0::size(1), 127::size(7), _::size(64), _::bits>>, _, _) do
    :error
  end

  def parse_header(<<_::size(8), 1::size(1), 127::size(7), _::size(96), _::bits>>, _, _) do
    :error
  end
  ...
  defp parse_header(opcode, fin, fragState, rsv, len, maskKey, rest) do
  ...
  def parse_payload(data, maskKey, utf8State, 0, type = :close, len, fragState, _, <<0::size(3)>>) do
    {<<maskedCode::size(2)-binary, data2::bits>>, rest, eof} = split_payload(data, len)
    <<closeCode::size(16)>> = unmask(maskedCode, maskKey, 0)

    case validate_close_code(closeCode) do
      :ok ->
        payload = unmask(data2, maskKey, 2)

        case validate_payload(payload, rest, utf8State, 2, type, fragState, eof) do
          {:ok, _, utf8State2, _} ->
  ...
  def parse_payload(data, maskKey, utf8State, parsedLen, type, len, fragState, _, <<0::size(3)>>) do
    {data2, rest, eof} = split_payload(data, len)
  ...
  defp validate_payload(payload, rest, utf8State, _, :fragment, {fin, :text, _}, eof) do
    case validate_utf8(payload, utf8State) do
      1 ->
        {:error, :badencoding}
  ...
  defp validate_utf8(<<c, rest::bits>>, 2)
       when c >= 128 and
              c < 144 do
    validate_utf8(rest, 0)
  end

  defp validate_utf8(<<c, rest::bits>>, 3)
       when c >= 128 and
              c < 144 do
    validate_utf8(rest, 2)
  end

  defp validate_utf8(<<c, rest::bits>>, 5)
       when c >= 128 and
              c < 144 do
    validate_utf8(rest, 2)
  end

  defp validate_utf8(<<c, rest::bits>>, 7)
       when c >= 128 and
              c < 144 do
    validate_utf8(rest, 3)
  end

  defp validate_utf8(<<c, rest::bits>>, 8)
       when c >= 128 and
              c < 144 do
    validate_utf8(rest, 3)
  end
  ...
  defp validate_utf8(<<c, rest::bits>>, 6)
       when c >= 144 and
              c < 160 do
    validate_utf8(rest, 3)
  end
  ...
  defp validate_utf8(<<c, rest::bits>>, 4)
       when c >= 160 and
              c < 192 do
    validate_utf8(rest, 2)
  end
  ...
  defp validate_utf8(<<c, rest::bits>>, 0)
       when c >= 194 and
              c < 224 do
    validate_utf8(rest, 2)
  end
  ...
  def frame(:close, _) do
    <<1::size(1), 0::size(3), 8::size(4), 0::size(8)>>
  end

  def frame(:ping, _) do
    <<1::size(1), 0::size(3), 9::size(4), 0::size(8)>>
  end

  def frame(:pong, _) do
    <<1::size(1), 0::size(3), 10::size(4), 0::size(8)>>
  end
  ...
  def frame({:close, statusCode, payload}, _) do
    ...
    [
      <<1::size(1), 0::size(3), 8::size(4), 0::size(1), len::size(7), statusCode::size(16)>>,
      payload
    ]
  end
  ...
  def frame({:ping, payload}, _) do
    ...
    true = len <= 125
    [<<1::size(1), 0::size(3), 9::size(4), 0::size(1), len::size(7)>>, payload]
  end

  def frame({:pong, payload}, _) do
    len = :erlang.iolist_size(payload)
    true = len <= 125
    [<<1::size(1), 0::size(3), 10::size(4), 0::size(1), len::size(7)>>, payload]
  end
  ...
  def frame(
    ...
    [<<1::size(1), 1::size(1), 0::size(2), 1::size(4), 0::size(1), len::bits>>, payload2]
  end
  ...
  def frame(
    ...
    payload2 = deflate_frame(payload, deflate, takeOver)
    len = payload_length(payload2)
    [<<1::size(1), 1::size(1), 0::size(2), 2::size(4), 0::size(1), len::bits>>, payload2]
  end

  def frame({:text, payload}, _) do
    len = payload_length(payload)
    [<<1::size(1), 0::size(3), 1::size(4), 0::size(1), len::bits>>, payload]
  end

  def frame({:binary, payload}, _) do
    len = payload_length(payload)
    [<<1::size(1), 0::size(3), 2::size(4), 0::size(1), len::bits>>, payload]
  end

  def masked_frame(:close, _) do
    <<1::size(1), 0::size(3), 8::size(4), 1::size(1), 0::size(39)>>
  end

  def masked_frame(:ping, _) do
    <<1::size(1), 0::size(3), 9::size(4), 1::size(1), 0::size(39)>>
  end

  def masked_frame(:pong, _) do
    <<1::size(1), 0::size(3), 10::size(4), 1::size(1), 0::size(39)>>
  end
  ...
  def masked_frame({:close, statusCode, payload}, _) do
    len = 2 + :erlang.iolist_size(payload)
    true = len <= 125
    maskKeyBin = <<maskKey::size(32)>> = :crypto.strong_rand_bytes(4)

    [
      <<1::size(1), 0::size(3), 8::size(4), 1::size(1), len::size(7)>>,
      maskKeyBin,
      mask(:erlang.iolist_to_binary([<<statusCode::size(16)>>, payload]), maskKey, <<>>)
    ]
  end
  ...
  def masked_frame({:ping, payload}, _) do
    ...
    [
      <<1::size(1), 0::size(3), 9::size(4), 1::size(1), len::size(7)>>,
      maskKeyBin,
      mask(:erlang.iolist_to_binary(payload), maskKey, <<>>)
    ]
  end
  ...
  def masked_frame({:pong, payload}, _) do
    ...
    [
      <<1::size(1), 0::size(3), 10::size(4), 1::size(1), len::size(7)>>,
      maskKeyBin,
      mask(:erlang.iolist_to_binary(payload), maskKey, <<>>)
    ]
  end
  ...
  def masked_frame(
    ...
    [
      <<1::size(1), 1::size(1), 0::size(2), 1::size(4), 1::size(1), len::bits>>,
      maskKeyBin,
      payload2
    ]
  end
  ...
  def masked_frame(
    ...
    [
      <<1::size(1), 1::size(1), 0::size(2), 2::size(4), 1::size(1), len::bits>>,
      maskKeyBin,
      payload2
    ]
  end
  ...
  def masked_frame({:text, payload}, _) do
    ...
    [
      <<1::size(1), 0::size(3), 1::size(4), 1::size(1), len::bits>>,
      maskKeyBin,
      mask(:erlang.iolist_to_binary(payload), maskKey, <<>>)
    ]
  end
  ...
  def masked_frame({:binary, payload}, _) do
    ...
    [
      <<1::size(1), 0::size(3), 2::size(4), 1::size(1), len::bits>>,
      maskKeyBin,
      mask(:erlang.iolist_to_binary(payload), maskKey, <<>>)
    ]
  end
  ...
  defp deflate_frame(payload, deflate, takeOver) do
    deflated = :erlang.iolist_to_binary(:zlib.deflate(deflate, payload, :sync))
    ...
    case deflated do
      <<body::size(len)-binary, 0::size(8), 0::size(8), 255::size(8), 255::size(8)>> ->
        body
