defmodule :cowboy_tracer_h do
  ...
  defp init_tracer(streamID, req, opts = %{:tracer_match_specs => list, :tracer_callback => _}) do
  ...
  defp match([{:method, value} | tail], streamID, req = %{:method => value}, opts) do
    match(tail, streamID, req, opts)
  end

  defp match([{:host, value} | tail], streamID, req = %{:host => value}, opts) do
    match(tail, streamID, req, opts)
  end

  defp match([{:path, value} | tail], streamID, req = %{:path => value}, opts) do
    match(tail, streamID, req, opts)
  end

  defp match([{:path_start, pathStart} | tail], streamID, req = %{:path => path}, opts) do
  ...
  defp match([{:header, name} | tail], streamID, req = %{:headers => headers}, opts) do
    case headers do
      %{^name => _} ->
        match(tail, streamID, req, opts)
  ...
  defp match([{:header, name, value} | tail], streamID, req = %{:headers => headers}, opts) do
    case headers do
      %{^name => ^value} ->
        match(tail, streamID, req, opts)
  ...
  defp match([{:peer_ip, iP} | tail], streamID, req = %{:peer => {iP, _}}, opts) do
    match(tail, streamID, req, opts)
  end
  ...
  def tracer_process(streamID, req = %{:pid => parent}, opts = %{:tracer_callback => fun}) do
    :erlang.process_flag(:trap_exit, true)
    state = fun.(:init, {streamID, req, opts})
    tracer_loop(parent, opts, state)
  end

  defp tracer_loop(parent, opts = %{:tracer_callback => fun}, state0) do
    receive do
      ...
      {:system, from, request} ->
        :sys.handle_system_msg(request, from, parent, :cowboy_tracer_h, [], {opts, state0})

      msg ->
        :cowboy.log(
          :warning,
          '~p: Tracer process received stray message ~9999p~n',
          [:cowboy_tracer_h, msg],
          opts
        )
