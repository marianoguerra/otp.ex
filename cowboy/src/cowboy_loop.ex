defmodule :cowboy_loop do
  ...
  def loop(req = %{pid: parent}, env, handler,
           handlerState) do
    receive do
      {:EXIT, ^parent, reason} ->
  ...
  defp call(req0, env, handler, handlerState0, message) do
    try do
      ...
      class, reason ->
        :cowboy_handler.terminate({:crash, class, reason}, req0,
                                    handlerState0, handler)
  ...
  defp suspend(req, env, handler, handlerState) do
    {:suspend, :cowboy_loop, :loop,
       [req, env, handler, handlerState]}
  end

  defp terminate(req, env, handler, handlerState, reason) do
    result = :cowboy_handler.terminate(reason, req,
  ...
  def system_terminate(reason, _, _,
           {req, env, handler, handlerState}) do
    terminate(req, env, handler, handlerState, reason)
  end
