defmodule :cowboy_websocket do
  ...
  Record.defrecord(:r_state, :state, parent: :undefined,
                                 ...
                                 transport: :undefined, opts: %{}, active: true,
                                 handler: :undefined, key: :undefined,
                                 timeout_ref: :undefined, messages: :undefined,
                                 hibernate: false, frag_state: :undefined,
                                 frag_buffer: <<>>, utf8_state: :undefined,
                                 deflate: true, extensions: %{}, req: %{},
  ...
  def upgrade(req0 = %{version: version}, env, handler,
           handlerState, opts) do
    filteredReq = (case (:maps.get(:req_filter, opts,
                                     :undefined)) do
                     :undefined ->
                       :maps.with([:method, :version, :scheme, :host, :port,
                                                                          :path,
                                                                              :qs,
                                                                                  :peer],
                                    req0)
    ...
    utf8State = (case (:maps.get(:validate_utf8, opts,
                                   true)) do
                   true ->
                     0
                   false ->
    ...
    state0 = r_state(opts: opts, handler: handler,
                 utf8_state: utf8State, req: filteredReq)
    try do
  ...
  defp websocket_version(state, req) do
    ...
    websocket_extensions(state,
                           Map.put(req, :websocket_version, wsVersion))
  end
  ...
  defp websocket_extensions(state, req, [], [", " | respHeader]) do
    {:ok, state,
       :cowboy_req.set_resp_header("sec-websocket-extensions",
                                     :lists.reverse(respHeader), req)}
  end

  defp websocket_extensions(state = r_state(opts: opts, extensions: extensions),
            req = %{pid: pid, version: version},
            [{"permessage-deflate", params} | tail], respHeader) do
    deflateOpts0 = :maps.get(:deflate_opts, opts, %{})
    deflateOpts = (case (version) do
                     :"HTTP/1.1" ->
                       Map.put(deflateOpts0, :owner, pid)
                     _ ->
                       deflateOpts0
                   end)
    try do
      :cow_ws.negotiate_permessage_deflate(params, extensions,
                                             deflateOpts)
    catch
      ...
      {:ok, respExt, extensions2} ->
        websocket_extensions(r_state(state, extensions: extensions2),
  ...
  defp websocket_extensions(state = r_state(opts: opts, extensions: extensions),
            req = %{pid: pid, version: version},
            [{"x-webkit-deflate-frame", params} | tail], respHeader) do
    deflateOpts0 = :maps.get(:deflate_opts, opts, %{})
    deflateOpts = (case (version) do
                     :"HTTP/1.1" ->
                       Map.put(deflateOpts0, :owner, pid)
    ...
    try do
      :cow_ws.negotiate_x_webkit_deflate_frame(params,
                                                 extensions, deflateOpts)
    catch
      :exit, {:error, :incompatible_zlib_version, _} ->
        websocket_extensions(state, req, tail, respHeader)
    else
  ...
  defp websocket_handshake(state = r_state(key: key),
            req = %{version: :"HTTP/1.1", pid: pid, streamid: streamID},
            handlerState, env) do
    challenge = :base64.encode(:crypto.hash(:sha,
                                              <<key :: binary, "258EAFA5-E914-47DA-95CA-C5AB0DC85B11">>))
    headers = :cowboy_req.response_headers(%{"connection" => "Upgrade", "upgrade" => "websocket",
                                               "sec-websocket-accept" => challenge},
                                             req)
    send(pid, {{pid, streamID},
                 {:switch_protocol, headers, :cowboy_websocket,
                    {state, handlerState}}})
    {:ok, req, env}
  end

  defp websocket_handshake(state,
            req = %{ref: ref, pid: pid, streamid: streamID},
            handlerState, _Env) do
    headers = :cowboy_req.response_headers(%{}, req)
    send(pid, {{pid, streamID},
                 {:switch_protocol, headers, :cowboy_websocket,
                    {state, handlerState}}})
    takeover(pid, ref, {pid, streamID}, :undefined,
               :undefined, <<>>, {state, handlerState})
  end

  Record.defrecord(:r_ps_header, :ps_header, buffer: <<>>)
  Record.defrecord(:r_ps_payload, :ps_payload, type: :undefined,
                                      len: :undefined, mask_key: :undefined,
                                      rsv: :undefined, close_code: :undefined,
                                      unmasked: <<>>, unmasked_len: 0,
                                      buffer: <<>>)
  def takeover(parent, ref, socket, transport, _Opts, buffer,
           {state0 = r_state(handler: handler), handlerState}) do
    :ranch.remove_connection(ref)
    messages = (case (transport) do
                  :undefined ->
                    :undefined
                  _ ->
                    transport.messages()
                end)
    state = loop_timeout(r_state(state0, parent: parent, 
                                     ref: ref,  socket: socket, 
                                     transport: transport,  key: :undefined, 
                                     messages: messages))
    case (:erlang.function_exported(handler,
                                      :websocket_init, 1)) do
      true ->
        handler_call(state, handlerState, r_ps_header(buffer: buffer),
                       :websocket_init, :undefined, &after_init/3)
      false ->
        after_init(state, handlerState, r_ps_header(buffer: buffer))
    end
  end

  defp after_init(state = r_state(active: true), handlerState,
            parseState) do
  ...
  defp maybe_read_body(r_state(socket: stream = {pid, _},
              transport: :undefined, active: true)) do
    readBodyRef = make_ref()
    send(pid, {stream,
                 {:read_body, self(), readBodyRef, :auto, :infinity}})
    :ok
  end
  ...
  defp passive(state = r_state(socket: socket, transport: transport,
  ...
  defp before_loop(state = r_state(hibernate: true), handlerState,
            parseState) do
    :proc_lib.hibernate(:cowboy_websocket, :loop,
                          [r_state(state, hibernate: false), handlerState,
                                                           parseState])
  end
  ...
  def loop(state = r_state(parent: parent, socket: socket,
                     messages: messages, timeout_ref: tRef),
           handlerState, parseState) do
    receive do
      ...
      message ->
        handler_call(state, handlerState, parseState,
                       :websocket_info, message, &before_loop/3)
    end
  end

  defp parse(state, handlerState, pS = r_ps_header(buffer: buffer),
            data) do
    parse_header(state, handlerState,
                   r_ps_header(pS, buffer: <<buffer :: binary, data :: binary>>))
  end

  defp parse(state, handlerState, pS = r_ps_payload(buffer: buffer),
            data) do
    parse_payload(state, handlerState, r_ps_payload(pS, buffer: <<>>),
                    <<buffer :: binary, data :: binary>>)
  end

  defp parse_header(state = r_state(opts: opts, frag_state: fragState,
                      extensions: extensions),
            handlerState, parseState = r_ps_header(buffer: data)) do
    maxFrameSize = :maps.get(:max_frame_size, opts,
                               :infinity)
    case (:cow_ws.parse_header(data, extensions,
                                 fragState)) do
      {_, _, _, _, :undefined, _} ->
        websocket_close(state, handlerState,
                          {:error, :badframe})
      {_, _, _, len, _, _} when len > maxFrameSize ->
        websocket_close(state, handlerState, {:error, :badsize})
      {type, fragState2, rsv, len, maskKey, rest} ->
        parse_payload(r_state(state, frag_state: fragState2),
                        handlerState,
  ...
  defp parse_payload(state = r_state(frag_state: fragState,
                      utf8_state: incomplete, extensions: extensions),
            handlerState,
            parseState = r_ps_payload(type: type, len: len, mask_key: maskKey,
                             rsv: rsv, unmasked: unmasked,
                             unmasked_len: unmaskedLen),
            data) do
    case (:cow_ws.parse_payload(data, maskKey, incomplete,
                                  unmaskedLen, type, len, fragState, extensions,
                                  rsv)) do
      {:ok, closeCode, payload, utf8State, rest} ->
        dispatch_frame(r_state(state, utf8_state: utf8State),
                         handlerState,
                         r_ps_payload(parseState, unmasked: <<unmasked :: binary,
                                                     payload :: binary>>, 
      ...
      {:more, closeCode, payload, utf8State} ->
        before_loop(r_state(state, utf8_state: utf8State),
                      handlerState,
                      r_ps_payload(parseState, len: len - byte_size(data), 
                                      close_code: closeCode, 
                                      unmasked: <<unmasked :: binary,
                                                    payload :: binary>>, 
  ...
  defp dispatch_frame(state = r_state(opts: opts, frag_state: fragState,
                      frag_buffer: soFar),
            handlerState,
            r_ps_payload(type: type0, unmasked: payload0,
                close_code: closeCode0),
            remainingData) do
    maxFrameSize = :maps.get(:max_frame_size, opts,
                               :infinity)
    case (:cow_ws.make_frame(type0, payload0, closeCode0,
                               fragState)) do
      ...
      {:fragment, :nofin, _, payload} ->
        parse_header(r_state(state, frag_buffer: <<soFar :: binary,
                                               payload :: binary>>),
                       handlerState, r_ps_header(buffer: remainingData))
      {:fragment, :fin, type, payload} ->
        handler_call(r_state(state, frag_state: :undefined, 
                                frag_buffer: <<>>),
                       handlerState, r_ps_header(buffer: remainingData),
                       :websocket_handle,
      ...
      frame = :ping ->
        ...
        handler_call(state, handlerState,
                       r_ps_header(buffer: remainingData), :websocket_handle, frame,
                       &parse_header/3)
      frame = {:ping, payload} ->
        transport_send(state, :nofin,
                         frame({:pong, payload}, state))
        handler_call(state, handlerState,
                       r_ps_header(buffer: remainingData), :websocket_handle, frame,
                       &parse_header/3)
      frame ->
  ...
  defp handler_call(state = r_state(handler: handler), handlerState,
            parseState, callback, message, nextState) do
    try do
      case (callback) do
        :websocket_init ->
  ...
  defp handler_call_result(state0, handlerState, parseState, nextState,
            commands) do
  ...
  defp commands([], state, data) do
    result = transport_send(state, :nofin,
                              :lists.reverse(data))
    {result, state}
  end

  defp commands([{:active, active} | tail],
            state0 = r_state(active: active0), data)
  ...
  defp commands([{:set_options, setOpts} | tail],
            state0 = r_state(opts: opts), data) do
    state = (case (setOpts) do
               %{idle_timeout: idleTimeout} ->
                 loop_timeout(r_state(state0, opts: Map.put(opts, :idle_timeout,
                                                              idleTimeout)))
               _ ->
                 state0
             end)
  ...
  defp commands([{:shutdown_reason, shutdownReason} | tail],
            state, data) do
    commands(tail,
               r_state(state, shutdown_reason: shutdownReason), data)
  end
  ...
  defp transport_send(r_state(socket: stream = {pid, _},
              transport: :undefined),
            isFin, data) do
  ...
  defp transport_send(r_state(socket: socket, transport: transport), _,
            data) do
  ...
  defp websocket_send_close(state, reason) do
    _ = (case (reason) do
           normal when normal === :stop or normal === :timeout ->
             transport_send(state, :fin,
                              frame({:close, 1000, <<>>}, state))
           {:error, :badframe} ->
             transport_send(state, :fin,
                              frame({:close, 1002, <<>>}, state))
           {:error, :badencoding} ->
             transport_send(state, :fin,
                              frame({:close, 1007, <<>>}, state))
           {:error, :badsize} ->
             transport_send(state, :fin,
                              frame({:close, 1009, <<>>}, state))
           {:crash, _, _} ->
             transport_send(state, :fin,
                              frame({:close, 1011, <<>>}, state))
           :remote ->
             transport_send(state, :fin, frame(:close, state))
           {:remote, code, _} ->
             transport_send(state, :fin,
                              frame({:close, code, <<>>}, state))
         end)
    :ok
  end

  defp frame(frame,
            r_state(deflate: false, extensions: extensions)) do
    :cow_ws.frame(frame,
                    Map.put(extensions, :deflate, false))
  end
  ...
  defp terminate(state = r_state(shutdown_reason: shutdown),
            handlerState, reason) do
  ...
  defp handler_terminate(r_state(handler: handler, req: req), handlerState,
            reason) do
    :cowboy_handler.terminate(reason, req, handlerState,
                                handler)
  end
