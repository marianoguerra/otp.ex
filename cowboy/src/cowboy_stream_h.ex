defmodule :cowboy_stream_h do
  ...
  def init(streamID, req = %{:ref => ref}, opts) do
    ...
    pid = :proc_lib.spawn_link(:cowboy_stream_h, :request_process, [req, env, middlewares])
    expect = expect(req)
  ...
  def data(
        ...
        isFin,
        data,
        state = r_state(read_body_ref: :undefined, read_body_buffer: buffer, body_length: bodyLen)
      ) do
    do_data(
      streamID,
      isFin,
  ...
  def data(
        ...
        isFin = :nofin,
        data,
        state = r_state(read_body_length: readLen, read_body_buffer: buffer, body_length: bodyLen)
      )
  ...
  defp do_data(streamID, isFin, data, commands1, state = r_state(next: next0)) do
    {commands2, next} = :cowboy_stream.data(streamID, isFin, data, next0)
    {commands1 ++ commands2, r_state(state, next: next)}
  end

  def info(streamID, info = {:EXIT, pid, :normal}, state = r_state(pid: pid)) do
  ...
  def info(
        ...
        state = r_state(read_body_buffer: <<>>)
      ) do
    do_info(
      streamID,
      info,
      [],
      r_state(state, read_body_pid: pid, read_body_ref: ref, read_body_length: :auto)
    )
  end
  ...
  def info(
        streamID,
        info = {:read_body, pid, ref, :auto, :infinity},
        state = r_state(read_body_is_fin: isFin, read_body_buffer: buffer, body_length: bodyLen)
      ) do
    send_request_body(pid, ref, isFin, bodyLen, buffer)
    do_info(streamID, info, [{:flow, byte_size(buffer)}], r_state(state, read_body_buffer: <<>>))
  end
  ...
  def info(
    ...
    send_request_body(pid, ref, isFin, bodyLen, buffer)
    do_info(streamID, info, [], r_state(state, read_body_buffer: <<>>))
  end

  def info(
        streamID,
        info = {:read_body, pid, ref, length, period},
        state = r_state(expect: expect)
      ) do
    commands =
      case expect do
        :continue ->
  ...
  def info(streamID, info = {:read_body_timeout, _}, state) do
    do_info(streamID, info, [], state)
  end

  def info(streamID, inform = {:inform, status, _}, state0) do
    state =
      case :cow_http.status_to_integer(status) do
        100 ->
  ...
  def info(streamID, response = {:response, _, _, _}, state) do
    do_info(streamID, response, [response], r_state(state, expect: :undefined))
  end

  def info(streamID, headers = {:headers, _, _}, state) do
    do_info(streamID, headers, [headers], r_state(state, expect: :undefined))
  end
  ...
  def info(streamID, data0 = {:data, pid, _, _}, state0 = r_state(stream_body_status: status)) do
    state =
      case status do
        :normal ->
  ...
  def info(streamID, alarm = {:alarm, name, :on}, state0 = r_state(stream_body_status: status))
  ...
  def info(streamID, push = {:push, _, _, _, _, _, _, _}, state) do
    do_info(streamID, push, [push], state)
  end

  def info(streamID, switchProtocol = {:switch_protocol, _, _, _}, state) do
    do_info(streamID, switchProtocol, [switchProtocol], r_state(state, expect: :undefined))
  end

  def info(streamID, setOptions = {:set_options, _}, state) do
  ...
  defp do_info(streamID, info, commands1, state0 = r_state(next: next0)) do
  ...
  def request_process(req, env, middlewares) do
    try do
      ...
      :exit, reason ->
        :erlang.raise(:exit, {reason, __STACKTRACE__}, __STACKTRACE__)
    end
  end
  ...
  defp execute(req, env, [middleware | tail]) do
    case middleware.execute(req, env) do
      ...
      {:suspend, module, function, args} ->
        :proc_lib.hibernate(:cowboy_stream_h, :resume, [env, tail, module, function, args])
  ...
  def resume(env, tail, module, function, args) do
    case apply(module, function, args) do
      ...
      {:suspend, module2, function2, args2} ->
        :proc_lib.hibernate(:cowboy_stream_h, :resume, [env, tail, module2, function2, args2])
