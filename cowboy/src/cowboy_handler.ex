defmodule :cowboy_handler do
  ...
  def execute(req,
           env = %{handler: handler, handler_opts: handlerOpts}) do
    try do
      handler.init(req, handlerOpts)
    catch
      class, reason ->
        terminate({:crash, class, reason}, req, handlerOpts,
                    handler)
        :erlang.raise(class, reason, __STACKTRACE__)
    else
      {:ok, req2, state} ->
        result = terminate(:normal, req2, state, handler)
        {:ok, req2, Map.put(env, :result, result)}
      {mod, req2, state} ->
  ...
  def terminate(reason, req, state, handler) do
    case (:erlang.function_exported(handler, :terminate,
                                      3)) do
      true ->
