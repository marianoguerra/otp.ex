defmodule :cowboy_compress_h do
  ...
  Record.defrecord(:r_state, :state, next: :undefined,
                                 threshold: :undefined, compress: :undefined,
                                 deflate: :undefined, deflate_flush: :sync)
  def init(streamID, req, opts) do
    state0 = check_req(req)
    compressThreshold = :maps.get(:compress_threshold, opts,
                                    300)
    deflateFlush = buffering_to_zflush(:maps.get(:compress_buffering,
                                                   opts, false))
    {commands0, next} = :cowboy_stream.init(streamID, req,
                                              opts)
    fold(commands0,
           r_state(state0, next: next,  threshold: compressThreshold, 
                       deflate_flush: deflateFlush))
  end

  def data(streamID, isFin, data,
           state0 = r_state(next: next0)) do
    {commands0, next} = :cowboy_stream.data(streamID, isFin,
                                              data, next0)
    fold(commands0, r_state(state0, next: next))
  end

  def info(streamID, info, state0 = r_state(next: next0)) do
    {commands0, next} = :cowboy_stream.info(streamID, info,
                                              next0)
    fold(commands0, r_state(state0, next: next))
  end
  ...
  defp fold([response = {:response, _, _,
                         {:sendfile, _, _, _}} |
               tail],
            state, acc) do
  ...
  defp fold([response0 = {:response, _, headers, body} |
               tail],
            state0 = r_state(threshold: compressThreshold), acc) do
  ...
  defp fold([response0 = {:headers, _, headers} | tail],
            state0, acc) do
    case (check_resp_headers(headers, state0)) do
      state = r_state(compress: :undefined) ->
        fold(tail, state, [response0 | acc])
  ...
  defp fold([data0 = {:data, _, _} | tail],
            state0 = r_state(compress: :gzip), acc) do
    {data, state} = gzip_data(data0, state0)
    fold(tail, state, [data | acc])
  end

  defp fold([trailers = {:trailers, _} | tail],
            state0 = r_state(compress: :gzip), acc) do
    {{:data, :fin, data}, state} = gzip_data({:data, :fin,
                                                <<>>},
                                               state0)
    fold(tail, state,
           [trailers, {:data, :nofin, data} | acc])
  end

  defp fold([setOptions = {:set_options, opts} | tail],
            state = r_state(threshold: compressThreshold0,
                        deflate_flush: deflateFlush0),
            acc) do
    compressThreshold = :maps.get(:compress_threshold, opts,
                                    compressThreshold0)
    deflateFlush = (case (opts) do
    ...
    fold(tail,
           r_state(state, threshold: compressThreshold, 
                      deflate_flush: deflateFlush),
           [setOptions | acc])
  end
  ...
  defp gzip_response({:response, status, headers, body}, state) do
    ...
    {{:response, status,
        vary(Map.merge(headers, %{"content-length"
                                  =>
                                  :erlang.integer_to_binary(:erlang.iolist_size(gzBody)),
  ...
  defp gzip_headers({:headers, status, headers0}, state) do
    ...
    {{:headers, status, vary(Map.put(headers, "content-encoding", "gzip"))},
  ...
  defp vary(headers = %{"vary" => vary}) do
    try do
      ...
      _, _ ->
        Map.put(headers, "vary", "accept-encoding")
    else
      :"*" ->
        headers
      list ->
        case (:lists.member("accept-encoding", list)) do
          true ->
            headers
          false ->
            Map.put(headers, "vary", [vary, ", accept-encoding"])
        end
    end
  end
  ...
  defp gzip_data({:data, :nofin,
             sendfile = {:sendfile, _, _, _}},
            state = r_state(deflate: z, deflate_flush: flush)) do
    {:ok, data0} = read_file(sendfile)
    data = :zlib.deflate(z, data0, flush)
    {{:data, :nofin, data}, state}
  end

  defp gzip_data({:data, :fin, sendfile = {:sendfile, _, _, _}},
            state = r_state(deflate: z)) do
    {:ok, data0} = read_file(sendfile)
  ...
  defp gzip_data({:data, :nofin, data0},
            state = r_state(deflate: z, deflate_flush: flush)) do
    data = :zlib.deflate(z, data0, flush)
    {{:data, :nofin, data}, state}
  end
