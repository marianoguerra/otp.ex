defmodule :cowboy_router do
  ...
  defp compile_paths([{pathMatch, handler, opts} | tail], acc) do
    compile_paths(
      [{pathMatch, [], handler, opts} | tail],
      acc
    )
  end

  defp compile_paths(
         [{pathMatch, fields, handler, opts} | tail],
         acc
       )
       when is_list(pathMatch) do
    compile_paths(
      [
        {:erlang.iolist_to_binary(pathMatch), fields, handler, opts}
        | tail
      ],
      acc
    )
  ...
  defp compile_paths(
         [
           {<<?/, pathMatch::bits>>, fields, handler, opts}
           | tail
         ],
         acc
       ) do
  ...
  defp compile_rules(<<s, rest::bits>>, s, segments, rules, <<>>) do
  ...
  defp compile_binding(rest = <<c, _::bits>>, s, acc)
       when c === s or
              c === ?[ or c === ?] do
    {acc, rest}
  end
  ...
  defp compile_brackets(rest, s, segments, rules) do
    ...
    rules2 = compile_rules(<<bracket::binary, rest2::binary>>, s, segments, [], <<>>)
    rules ++ rules2 ++ rules1
  end
  ...
  defp compile_brackets_split(<<c, rest::bits>>, acc, n)
       when c === ?] and
              n > 0 do
    compile_brackets_split(rest, <<acc::binary, c>>, n - 1)
  end
  ...
  def execute(
    ...
    case match(dispatch, host, path) do
      {:ok, handler, handlerOpts, bindings, hostInfo, pathInfo} ->
        {:ok, %{req | :host_info => hostInfo, :path_info => pathInfo, :bindings => bindings},
  ...
  defp match([{hostMatch, fields, pathMatchs} | tail], tokens, path)
  ...
  defp match_path([{:_, [], handler, opts} | _Tail], hostInfo, _, bindings) do
    {:ok, handler, opts, bindings, hostInfo, :undefined}
  end

  defp match_path([{"*", _, handler, opts} | _Tail], hostInfo, "*", bindings) do
    {:ok, handler, opts, bindings, hostInfo, :undefined}
  end
  ...
  defp match_path([{pathMatch, fields, handler, opts} | tail], hostInfo, tokens, bindings)
       when is_list(tokens) do
