defmodule :cowboy_rest do
  ...
  def upgrade(req0, env, handler, handlerState0) do
    ...
    case service_available(
           req0,
           r_state(method: method, handler: handler, handler_state: handlerState0)
         ) do
      {:ok, req, result} ->
  ...
  defp service_available(req, state) do
    expect(req, state, :service_available, true, &known_methods/2, 503)
  end

  defp known_methods(req, state = r_state(method: method)) do
    case call(req, state, :known_methods) do
      :no_call
      when method === "HEAD" or method === "GET" or
  ...
  defp uri_too_long(req, state) do
    expect(req, state, :uri_too_long, false, &allowed_methods/2, 414)
  end
  ...
  defp allowed_methods(req, state = r_state(method: method)) do
    case call(req, state, :allowed_methods) do
      ...
      :no_call when method === "OPTIONS" ->
        next(
          req,
          r_state(state, allowed_methods: ["HEAD", "GET", "OPTIONS"]),
          &malformed_request/2
        )
      ...
      {list, req2, state2} ->
        case :lists.member(method, list) do
          true when method === "OPTIONS" ->
            next(req2, r_state(state2, allowed_methods: list), &malformed_request/2)
  ...
  defp malformed_request(req, state) do
    expect(req, state, :malformed_request, false, &is_authorized/2, 400)
  end
  ...
  defp forbidden(req, state) do
    expect(req, state, :forbidden, false, &rate_limited/2, 403)
  end
  ...
  defp valid_content_headers(req, state) do
    expect(req, state, :valid_content_headers, true, &valid_entity_length/2, 501)
  end

  defp valid_entity_length(req, state) do
    expect(req, state, :valid_entity_length, true, &options/2, 413)
  end

  defp options(
         req,
         state = r_state(allowed_methods: methods, method: "OPTIONS")
       ) do
  ...
  defp content_types_provided(req, state) do
    case call(req, state, :content_types_provided) do
      :no_call ->
        state2 = r_state(state, content_types_p: [{{"text", "html", :*}, :to_html}])
        ...
        try do
          ...
          :undefined ->
            languages_provided(
              ...
              r_state(state2, content_type_a: {{"text", "html", []}, :to_html})
            )
  ...
  defp prioritize_mediatype(
         {typeA, subTypeA, paramsA},
         {typeB, subTypeB, paramsB}
       ) do
    case typeB do
      ^typeA ->
        case subTypeB do
  ...
  defp choose_media_type(req, state = r_state(content_types_p: cTP), [mediaType | tail]) do
  ...
  defp match_media_type(req, state, accept, cTP, mediaType = {{"*", "*", _Params_A}, _QA, _APA}) do
    ...
    case :lists.keytake("charset", 1, params_A0) do
      {:value, {_, charset}, params_A} when tA === "text" ->
        case call(req, state, :charsets_provided) do
          :no_call ->
            languages_provided(
              %{req | :media_type => {tP, sTP, params_A0}},
          ...
          {cP, req2, state2} ->
            ...
            case :lists.member(charset, cP) do
              ...
              true ->
                languages_provided(
                  %{req2 | :media_type => {tP, sTP, params_A}},
                  r_state(state3,
  ...
  defp match_media_type_params(
    ...
    case :lists.sort(params_P0) === :lists.sort(params_A) do
      true when tP === "text" ->
        {charset, params_P} =
          case :lists.keytake("charset", 1, params_P0) do
            false ->
              {:undefined, params_P0}
        ...
        languages_provided(
          %{req | :media_type => {tP, sTP, params_P}},
          r_state(state,
            content_type_a: {{tP, sTP, params_P}, fun},
            charset_a: charset
          )
        )
  ...
  defp prioritize_languages(acceptLanguages) do
    :lists.sort(
      fn {_TagA, qualityA}, {_TagB, qualityB} ->
        qualityA > qualityB
      end,
      acceptLanguages
    )
  ...
  defp choose_language(req, state = r_state(languages_p: lP), [language | tail]) do
  ...
  defp match_language(req, state, _Accept, [provided | _Tail], {:*, _Quality}) do
    set_language(req, r_state(state, language_a: provided))
  end

  defp match_language(req, state, _Accept, [provided | _Tail], {provided, _Quality}) do
    set_language(req, r_state(state, language_a: provided))
  end

  defp match_language(req, state, accept, [provided | tail], language = {tag, _Quality}) do
  ...
  defp prioritize_charsets(acceptCharsets) do
    :lists.sort(
      fn {_CharsetA, qualityA}, {_CharsetB, qualityB} ->
        qualityA > qualityB
      end,
      acceptCharsets
    )
  ...
  defp choose_charset(req, state = r_state(charsets_p: cP), [charset | tail]) do
  ...
  defp match_charset(req, state, _Accept, [provided | _], {provided, _}) do
    set_content_type(req, r_state(state, charset_a: provided))
  end
  ...
  defp variances(
         req,
         state = r_state(content_types_p: cTP, languages_p: lP, charsets_p: cP)
       ) do
    variances =
      case cTP do
        [] ->
    ...
    try do
      ...
      class, reason ->
        error_terminate(req, state, class, reason, __STACKTRACE__)
    else
      {variances4, req2, state2} ->
        case (for v <- variances4 do
          ...
          [[", ", h] | variances5] ->
            req3 = :cowboy_req.set_resp_header("vary", [h | variances5], req2)
  ...
  defp resource_exists(req, state) do
    expect(req, state, :resource_exists, true, &if_match_exists/2, &if_match_must_not_exist/2)
  end
    ...
    try do
      ...
      {req3, state2} ->
        try do
          ...
          class, reason ->
            error_terminate(req, state2, class, reason, __STACKTRACE__)
        else
          {req4, state3} ->
  ...
  defp allow_missing_post(req, state, onFalse) do
    expect(req, state, :allow_missing_post, true, &accept_resource/2, onFalse)
  end
  ...
  defp delete_resource(req, state) do
    expect(req, state, :delete_resource, false, 500, &delete_completed/2)
  end

  defp delete_completed(req, state) do
    expect(req, state, :delete_completed, true, &has_resp_body/2, 202)
  end

  defp is_conflict(req, state) do
    expect(req, state, :is_conflict, false, &accept_resource/2, 409)
  end
  ...
  defp choose_content_type(req, state, contentType, [{accepted, fun} | _Tail])
  ...
  defp process_content_type(req, state = r_state(method: method, exists: exists), fun) do
    try do
      ...
      {lastModified, req2, state2} ->
        case lastModified do
          ...
          ^lastModified ->
            ...
            req3 = :cowboy_req.set_resp_header("last-modified", lastModifiedBin, req2)
  ...
  defp if_range(
         req = %{:headers => %{"if-range" => _, "range" => _}},
         state = r_state(etag: etag)
       ) do
    try do
  ...
  defp choose_range(req, state = r_state(ranges_a: rangesAccepted), range = {rangeUnit, _}) do
  ...
  defp range_satisfiable(req, state, callback) do
    case call(req, state, :range_satisfiable) do
      ...
      {{false, int}, req2, state2} when is_integer(int) ->
        range_not_satisfiable(req2, state2, ["*/", :erlang.integer_to_binary(int)])
  ...
  defp set_ranged_body_auto(
         req,
         state =
           r_state(
             handler: handler,
             content_type_a: {_, callback}
           )
       ) do
    try do
  ...
  defp maybe_set_ranged_body_auto(req = %{:range => {_, ranges}}, state, body) do
    ...
    case :lists.usort(checks) do
      ...
      _ ->
        range_not_satisfiable(req, state, ["*/", :erlang.integer_to_binary(size)])
    end
  end
  ...
  defp ranged_partition(range, {:sendfile, offset0, bytes0, path}) do
    {from, to, offset, bytes} =
      case range do
        {from0, :infinity} ->
          {from0, bytes0 - 1, offset0 + from0, bytes0 - from0}

        {from0, to0} ->
          {from0, to0, offset0 + from0, 1 + to0 - from0}

        neg ->
          {bytes0 + neg, bytes0 - 1, offset0 + bytes0 + neg, -neg}
      end
    ...
    try do
      case call(req, state, callback) do
        ...
        {ranges, req2, state2} when length(ranges) > 1 ->
          hasSendfile =
            [] !==
              for {_, {:sendfile, _, _, _}} <- ranges do
                true
              end
  ...
  defp set_multipart_ranged_body(req, state, [firstRange | moreRanges]) do
    ...
    {firstContentRange, firstPartBody} =
      prepare_range(
        req,
        firstRange
      )
    ...
    firstPartHead =
      :cow_multipart.first_part(
        boundary,
        [{"content-type", contentType}, {"content-range", firstContentRange}]
      )

    moreParts =
      for nextRange <- moreRanges do
        {nextContentRange, nextPartBody} =
          prepare_range(
            req,
            nextRange
          )
        ...
        nextPartHead =
          :cow_multipart.part(
            boundary,
            [{"content-type", contentType}, {"content-range", nextContentRange}]
          )
  ...
  defp range_not_satisfiable(req0 = %{:range => {rangeUnit, _}}, state, rangeData) do
    req = :cowboy_req.set_resp_header("content-range", [rangeUnit, ?\s, rangeData], req0)
    respond(req, state, 416)
  end
  ...
  defp set_resp_etag(req, state) do
    ...
    case etag do
      ...
      ^etag ->
        req3 = :cowboy_req.set_resp_header("etag", encode_etag(etag), req2)
        {req3, state2}
    end
  end
  ...
  defp last_modified(req, state = r_state(last_modified: :undefined)) do
    case unsafe_call(req, state, :last_modified) do
      ...
      {lastModified, req2, state2} ->
        {lastModified, req2, r_state(state2, last_modified: lastModified)}
    end
  end
  ...
  defp expect(req, state, callback, expected, onTrue, onFalse) do
    case call(req, state, callback) do
      :no_call ->
  ...
  defp call(
    ...
    case :erlang.function_exported(handler, callback, 2) do
      true ->
        try do
          apply(handler, callback, [req0, handlerState0])
        catch
          class, reason ->
            error_terminate(req0, state, class, reason, __STACKTRACE__)
        else
          :no_call ->
            :no_call
  ...
  defp unsafe_call(
    ...
    case :erlang.function_exported(handler, callback, 2) do
      ...
      true ->
        case apply(handler, callback, [req0, handlerState0]) do
          :no_call ->
            :no_call
  ...
  defp respond(req0, state, statusCode) do
    req =
      case :cowboy_req.has_resp_body(req0) do
  ...
  defp switch_handler({:switch_handler, mod}, req, r_state(handler_state: handlerState)) do
    {mod, req, handlerState}
  end

  defp switch_handler({:switch_handler, mod, opts}, req, r_state(handler_state: handlerState)) do
    {mod, req, handlerState, opts}
  end

  defp error_terminate(
         req,
         r_state(handler: handler, handler_state: handlerState),
         class,
         reason,
         stacktrace
       ) do
